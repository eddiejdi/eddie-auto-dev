# Plano de Ação Executivo - Reorganização Eddie Auto-Dev
...
python3 refactor_lote1.py  # Remover "eddie_*" → "crypto_*"

# 2. Refatorar nomes em LOTE 2 (88 arquivos)  
python3 refactor_lote2.py  # Remover "eddie_*" → "homelab_*"
...
- 228 referências EDDIE removidas
...
# 3. Atualizar references no eddie-auto-dev
...
```python
# ANTES
eddie_trading_config = load_config()
EDDIE_API_KEY = os.getenv("EDDIE_API_KEY")
eddie_logger = get_logger("eddie")
...
```python
# ANTES
from btc_trading_agent.eddie import Trading
from eddie_tray_agent import config
...
```python
# ANTES
config["eddie_mode"] = "trading"
os.environ["EDDIE_HOME"] = "/home/eddie"
...
```
eddie-auto-dev/                    (Orchestrador - Framework principal)
├── specialized_agents/            (API + Message Bus + Core agents)
├── estou-aqui/                    (Mantém-se como submodule)
├── smartlife_integration/         (Integração IoT)
├── tools/                         (Libs compartilhadas - sem EDDIE refs)
├── scripts/                       (Utilities - sem EDDIE refs)
├── tests/
│   ├── unit/
│   ├── integration/
│   └── e2e/
...
### Manutenibilidade
- **Antes**: 228 refs EDDIE espalhadas
- **Depois**: 0 refs EDDIE (100% removidas)
...
|-----|------|-------|--------|
| Refs EDDIE removidas | 228 | 228 | ✅ Mapeado |
