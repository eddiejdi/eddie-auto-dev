...
from trading_engine import get_engine, EngineState
from btc_query import get_btc_price, get_market_analysis, get_recent_trades, get_performance
    ...
    def do_GET(self):
        ...
        # Saldo específico de uma moeda
            ...
            try:
                ...
                self._send_json({
                    "currency": currency,
                    "available": balance,
                    "formatted": f"{balance:.8f}" if currency == "BTC" else f"{balance:.2f}"
                })
            except Exception as e:
                self._send_error(f"Erro ao obter saldo: {str(e)}", 500)
        ...
        # Histórico de execuções (fills)
            ...
            try:
                ...
                # Calcular totais
                total_bought = sum(float(f.get("size", 0)) for f in fills if f.get("side") == "buy")
                total_sold = sum(float(f.get("size", 0)) for f in fills if f.get("side") == "sell")
