...
class RegimeAdjuster:
    ...
    def _calculate_ai_gating(self, adj: RegimeAdjustment) -> None:
        ...
        if regime == "BULLISH":
            ...
            adj.ai_aggressiveness = 0.6 + 0.3 * conf  # 0.6 – 0.9
            adj.ai_min_confidence = max(0.50, 0.65 - 0.15 * conf)  # 50% – 65%
            adj.ai_min_trade_interval = max(60, int(180 - 100 * conf))  # 80s – 180s
            ...
            adj.ai_aggressiveness = max(0.1, 0.4 - 0.2 * conf)  # 0.2 – 0.4
            adj.ai_min_confidence = min(0.80, 0.65 + 0.10 * conf)  # 65% – 75%
            adj.ai_min_trade_interval = min(600, int(180 + 200 * conf))  # 180s – 380s
            adj.ai_rebuy_lock_enabled = True
            adj.ai_rebuy_margin_pct = 0.002 * conf  # 0% – 0.2% de margem extra
            ...
            adj.ai_aggressiveness = 0.5 + 0.1 * ret_signal
            adj.ai_min_confidence = 0.60  # Padrão neutro
            adj.ai_min_trade_interval = 120  # 2 minutos
            adj.ai_rebuy_lock_enabled = True
            adj.ai_rebuy_margin_pct = 0.001  # 0.1% margem — compra perto do nível
        ...
        if adj.similar_count >= 10:
            ...
            adj.ai_min_confidence = max(0.45, adj.ai_min_confidence - 0.05)
    ...
    def _calculate_ai_buy_target(self, adj: RegimeAdjustment,
        ...
        try:
            ...
            if regime == "BULLISH":
                ...
                regime_discount = 0.003 + (0.007 * bear_severity * confidence)
                target = current_price * (1.0 - regime_discount)
