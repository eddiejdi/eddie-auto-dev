...
MIN_TRADE_INTERVAL = 60  # segundos mínimo entre trades
MIN_CONFIDENCE = 0.5  # confiança mínima para executar trade
...
class BitcoinTradingAgent:
    ...
    def _check_can_trade(self, signal: Signal) -> bool:
        ...
        if elapsed < MIN_TRADE_INTERVAL:
            logger.debug(
                f"⏳ Trade cooldown: {MIN_TRADE_INTERVAL - elapsed:.0f}s remaining"
            )
            return False
            ...
            try:
                if signal.action == "BUY":
                    ...
                    if self.state.dry_run:
                        ...
                        logger.info(
                            f"🔵 [DRY] BUY {size:.6f} BTC @ ${price:,.2f} (${amount_usdt:.2f})"
                        )
                    else:
                        # Trade real
                        result = place_market_order(
                            self.symbol, "buy", funds=amount_usdt
                        )
                    ...
                    if self.state.dry_run:
                        logger.info(
                            f"🔴 [DRY] SELL {size:.6f} BTC @ ${price:,.2f} "
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if signal.action != "HOLD" and self._check_can_trade(signal):
                    ...
                    if executed:
                        self.db.mark_decision_executed(
                            decision_id, self.state.total_trades
                        )
                ...
                if cycle % 60 == 0:  # A cada ~5 minutos
                    ...
                    logger.info(
                        f"📊 Cycle {cycle} | ${market_state.price:,.2f} | "
    ...
    def get_status(self) -> Dict:
        ...
        return {
            **self.state.to_dict(),
            ...
            "uptime_hours": (
                (time.time() - self.state.last_trade_time) / 3600
...
def main():
    ...
    def on_signal(sig: Signal):
        if sig.action != "HOLD":
            logger.info(
                f"📍 {sig.action} signal @ ${sig.price:,.2f} "
