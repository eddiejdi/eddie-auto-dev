...
from whatsapp_notifications import notify_buy, notify_sell, notify_error
...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler(LOG_DIR / "engine.log"), logging.StreamHandler()],
)
...
DEFAULT_CONFIG = {
    ...
    "max_daily_trades": 20,
    "max_daily_loss": 100,
    "trading_hours": {"enabled": False, "start": "09:00", "end": "22:00"},
    "notifications": {
        "enabled": True,
        "telegram_chat_id": "",
...
class TradingEngine:
    ...
    def _check_daily_limits(self) -> bool:
        ...
        # Limite de perda
        if (
            abs(self.stats.daily_pnl) >= self.config["max_daily_loss"]
            and self.stats.daily_pnl < 0
        ):
    ...
    def _calculate_position_size(self, signal: Signal, price: float) -> float:
        ...
        if balance <= 0 or balance < min_amt:
            logger.warning(
                f"⚠️ Saldo insuficiente em USDT: {balance:.6f} < min_trade_amount: {min_amt}"
            )
            return 0.0
        ...
        if amount < min_amt:
            logger.warning(
                f"⚠️ Amount calculado ({amount:.2f}) menor que min_trade_amount ({min_amt}); recusando."
            )
            return 0.0
    ...
    def _execute_buy(
        self, price: float, amount: float, trade_type: TradeType = TradeType.AUTO
    ) -> TradeResult:
        """Executa compra"""
        with self._trade_lock:
            try:
                ...
                if amount <= 0:
                    logger.warning(
                        "⚠️ Buy recusada: amount calculado é 0 ou menor (insuficiente / abaixo do mínimo)"
                    )
                    return TradeResult(
                        success=False,
                        error="Insufficient funds or amount below min_trade_amount",
                    )
                ...
                if not self.config["dry_run"]:
                    ...
                    if amount > available:
                        logger.warning(
                            f"⚠️ Buy recusada: amount pedido {amount:.2f} > saldo disponível {available:.2f}"
                        )
                        return TradeResult(
                ...
                if self.config["dry_run"]:
                    ...
                    result = place_market_order(
                        self.config["symbol"], "buy", funds=amount
                    )
    ...
    def _execute_sell(
        self, price: float, trade_type: TradeType = TradeType.AUTO
    ) -> TradeResult:
        """Executa venda"""
        with self._trade_lock:
            try:
                ...
                pnl_pct = (
                    ((price / self.stats.entry_price) - 1) * 100
                    if self.stats.entry_price > 0
                    else 0
                )
                ...
                if self.config["dry_run"]:
                    logger.info(
                        f"🔴 [DRY] SELL {size:.6f} BTC @ ${price:,.2f} (PnL: ${pnl:.2f})"
                    )
                else:
                    result = place_market_order(
                        self.config["symbol"], "sell", size=size
                    )
                    if not result.get("success"):
                        return TradeResult(success=False, error=str(result))
...
def main():
    ...
    parser.add_argument(
        "command", choices=["start", "stop", "status", "config"], help="Command"
    )
