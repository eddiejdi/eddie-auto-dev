...
class TrainingDatabase:
    ...
    def _init_db(self):
        ...
        with self._get_conn() as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_candles_lookup ON candles(symbol, ktype, timestamp)"
            )
    ...
    def get_recent_trades(
        self, symbol: str = None, limit: int = 100, include_dry: bool = False
    ) -> List[Dict]:
        ...
        with self._get_conn() as conn:
            ...
            """,
                (
                    ...
                    kwargs.get("ask"),
                    kwargs.get("spread"),
                    kwargs.get("orderbook_imbalance"),
                    kwargs.get("trade_flow"),
                    kwargs.get("rsi"),
                    kwargs.get("momentum"),
    ...
    def cleanup_old_data(self, days: int = 30):
        ...
        with self._get_conn() as conn:
            ...
            cursor.execute(
                "DELETE FROM learning_rewards WHERE timestamp < ?", (cutoff,)
            )
...
class TrainingManager:
    ...
    def generate_training_batch(self, symbol: str, batch_size: int = 100) -> List[Dict]:
...
if __name__ == "__main__":
    ...
    trade_id = db.record_trade(
        symbol="BTC-USDT", side="buy", price=95000.0, funds=100.0, dry_run=True
    )
    ...
    # Testar reward
    db.record_reward(
        symbol="BTC-USDT", state_hash="abc123", action=1, reward=0.5, episode=1
    )
