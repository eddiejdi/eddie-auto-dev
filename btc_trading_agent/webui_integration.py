...
"""
        ...
        if any(
            w in q
            for w in ["sinal", "decisão", "recomendação", "devo", "comprar", "vender"]
        ):
            analysis = self.get_market_analysis()
...
"""
        ...
        # Indicadores
        if any(
            w in q for w in ["indicador", "rsi", "momentum", "volatilidade", "técnico"]
        ):
            analysis = self.get_market_analysis()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="BTC Trading Agent - WebUI Integration"
    )
