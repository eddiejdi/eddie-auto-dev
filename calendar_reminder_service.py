...
# Configurações
REMINDER_MINUTES = [
    int(x) for x in os.getenv("CALENDAR_REMINDER_MINUTES", "30,10,5").split(",")
]
...
class ReminderState:
    ...
    def should_send_weekly_digest(self) -> bool:
        ...
        return (
            self.last_weekly_digest != week_key and today.weekday() == WEEKLY_DIGEST_DAY
        )
...
Prepare-se! 🚀"""
    ...
    async def send_weekly_digest(self):
        ...
        try:
            ...
            success, _, events = await self.calendar.list_events(
                time_min=week_start, time_max=week_end, max_results=50
            )
    ...
    async def run(self):
        ...
        if not self.calendar.is_authenticated():
            logger.error(
                "❌ Não autenticado! Execute setup_google_calendar.py primeiro."
            )
            return
