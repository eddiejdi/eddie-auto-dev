...
class DockerManager:
    ...
    def run_code(
        self, code: str, language: str = "python", timeout: int = None
    ) -> RunResult:
        timeout = timeout or self.timeout

        with tempfile.TemporaryDirectory() as tmpdir:
            ext = {"python": ".py", "javascript": ".js", "sql": ".sql"}.get(
                language, ".py"
            )
            filename = f"main{ext}"
    ...
    def run_tests(
        self, code: str, test_code: str, language: str = "python"
    ) -> RunResult:
        with tempfile.TemporaryDirectory() as tmpdir:
...
"""
            ...
            try:
                ...
                build_proc = subprocess.run(
                    ["docker", "build", "-t", safe_tag, tmpdir],
                    capture_output=True,
                    text=True,
                    timeout=300,
                )
