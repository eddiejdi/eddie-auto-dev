...
def fix():
    ...
    suggestion = None
    joined = "\n".join(errors).lower()
    if 'pytest' in joined or 'no module named' in joined or 'module not found' in joined:
        suggestion = 'Instale dependências faltantes: `pip install pytest` e reexecute os testes.'
    elif 'permission denied' in joined:
        suggestion = 'Verifique permissões e execute com usuário correto ou corrija owner/perms.'
    elif 'docker' in joined:
        suggestion = 'Verifique se o Docker está instalado e o usuário pertence ao grupo docker.'
    else:
