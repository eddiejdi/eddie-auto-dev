...
func TestCreateJiraIssue(t *testing.T) {
	...
	expectedID := fmt.Sprintf("%s-%d", projectKey, 100)
	expectedSummary := "Test Issue"
	expectedDescription := "This is a test issue created by Go Agent."

	if createdIssue.ID != expectedID || createdIssue.Summary != expectedSummary || createdIssue.Description != expectedDescription {
		t.Errorf("Created issue does not match expected values: %+v", createdIssue)
	}
}

// TestCreateJiraIssueError tests the createJiraIssue function with invalid data.
