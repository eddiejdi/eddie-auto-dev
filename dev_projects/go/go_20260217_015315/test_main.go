...
// createJiraIssue sends a POST request to create a new Jira issue and returns the created issue or an error
...
func main() {
	...
	err = json.Unmarshal(bodyBytes, &issue)
	if err != nil {
		fmt.Println("Error parsing JSON response: %v", err)
		return
	}
