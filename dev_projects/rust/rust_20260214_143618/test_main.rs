use reqwest;
use serde_json;

#[derive(Debug)]
struct JiraIssue {
    key: String,
    summary: String,
    status: String,
}

async fn get_jira_issue(jira_url: &str, issue_key: &str) -> Result<JiraIssue, reqwest::Error> {
    let url = format!("{}rest/api/2/issue/{}", jira_url, issue_key);
    let response = reqwest::get(&url).await?;
    if !response.status().is_success() {
        return Err(reqwest::Error::from(response));
    }
    let json: serde_json::Value = response.json().await?;
    Ok(JiraIssue {
        key: json["key"].as_str().unwrap().to_string(),
        summary: json["fields"]["summary"].as_str().unwrap().to_string(),
        status: json["fields"]["status"]["name"].as_str().unwrap().to_string(),
    })
}

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let jira_url = "https://your-jira-instance.atlassian.net";
    let issue_key = "ABC-123";

    match get_jira_issue(jira_url, issue_key).await {
        Ok(issue) => println!("Issue: {:?}", issue),
        Err(e) => eprintln!("Error: {}", e),
    }

    Ok(())
}

// Testes para a função get_jira_issue

#[tokio::test]
async fn test_get_jira_issue_success() {
    let jira_url = "https://your-jira-instance.atlassian.net";
    let issue_key = "ABC-123";

    let response = reqwest::get(&format!("{}rest/api/2/issue/{}", jira_url, issue_key)).await;
    assert!(response.is_ok());

    let json: serde_json::Value = response.json().await.unwrap();
    let expected_issue = JiraIssue {
        key: "ABC-123".to_string(),
        summary: "Test Issue".to_string(),
        status: "Open".to_string(),
    };

    assert_eq!(json["key"].as_str().unwrap(), &expected_issue.key);
    assert_eq!(json["fields"]["summary"].as_str().unwrap(), &expected_issue.summary);
    assert_eq!(json["fields"]["status"]["name"].as_str().unwrap(), &expected_issue.status);
}

#[tokio::test]
async fn test_get_jira_issue_error() {
    let jira_url = "https://your-jira-instance.atlassian.net";
    let issue_key = "ABC-123";

    let response = reqwest::get(&format!("{}rest/api/2/issue/{}", jira_url, issue_key)).await;
    assert!(response.is_err());

    match response {
        Err(e) => assert_eq!(e.status(), 404),
        Ok(_) => panic!("Expected an error"),
    }
}