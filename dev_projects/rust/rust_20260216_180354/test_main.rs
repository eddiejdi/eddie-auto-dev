...
mod tests {
    ...
    #[tokio::test]
    async fn test_monitor_issues_success() {
        let client = Client::new();
        let (tx, rx) = mpsc::channel(10);

        tokio::spawn(async move {
            monitor_issues(&client, tx).await.unwrap();
        });
    ...
    #[tokio::test]
    async fn test_monitor_issues_failure() {
        let client = Client::new();
        let (tx, rx) = mpsc::channel(10);

        tokio::spawn(async move {
            monitor_issues(&client, tx).await.unwrap_err();
        });
