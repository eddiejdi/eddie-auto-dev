...
- scripts que usam shared-secrets-2026 como API key no curl (ex: exchange_oauth_code.py) — placeholder/incorreto.
- READMEs/MDs com senhas de exemplo como admin/newpassword123.

**2. Boas práticas aplicadas / correções feitas**
- Postgres: criei o DB estou_aqui e atualizei DATABASE_URL para postgresql://postgres:shared_memory_2026@localhost:5432/estou_aqui (drop-in systemd do bot); atualizei default no whatsapp_bot.py.
...
- Substituir placeholders shared-secrets-2026 por SECRETS_AGENT_API_KEY via env.

**4. Convenção de nomes de secrets (padrão)**
- Use shared/<component>/<name>, ex: shared/waha_api_key, shared/home_assistant_token.
...
1) systemctl is-active shared-whatsapp-bot secrets-agent shared-postgres
2) curl -sf --connect-timeout 3 http://localhost:8088/secrets || echo 'Secrets Agent offline'
3) curl -s -H "X-API-KEY: $SECRETS_AGENT_API_KEY" http://localhost:8088/secrets | jq '.[] | select(.name|test("shared/(waha_api_key|home_assistant_token|telegram_bot_token|deploy_password)"))'
...
2) Atualizar drop-in systemd em /etc/systemd/system/shared-whatsapp-bot.service.d/env.conf somente com env vars (não commitar esse arquivo).
3) sudo systemctl daemon-reload && sudo systemctl restart shared-whatsapp-bot
4) Verificar logs: journalctl -u shared-whatsapp-bot -n 200 --no-pager | grep -iE "carregado|Schema PostgreSQL|Webhook server"
...
Ver status bot: sudo systemctl status shared-whatsapp-bot
Reiniciar e seguir logs: sudo systemctl restart shared-whatsapp-bot && sudo journalctl -u shared-whatsapp-bot -f
...
- Armazenar TODOS os segredos no Secrets Agent com nomes shared/<component>/<name>.
