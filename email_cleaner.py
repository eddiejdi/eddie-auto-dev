...
WHITELIST = ["nubank", "itau", "bradesco", "gov.br", "github.com", "google.com", "microsoft.com", "openai.com", "anthropic.com", "paypal", "mercadopago"]
IMPORTANT_SUBJECTS = ["pagamento", "fatura", "boleto", "senha", "password", "contrato", "urgente", "important", "reuniao", "servidor", "deploy", "erro", "pull request"]
BLACKLIST = ["mailchimp", "sendgrid", "newsletter", "noreply", "no-reply", "marketing@", "promo@", "amazon.com.br", "mercadolivre", "shopee", "aliexpress", "facebookmail", "twitter", "linkedin", "instagram", "netflix", "spotify", "uber.com", "ifood", "rappi", "medium.com", "udemy"]

class GmailCleaner:
    def __init__(self, dry_run=True):
        self.dry_run = dry_run
        self.service = None
        self.stats = {"scanned": 0, "to_delete": 0, "deleted": 0, "skipped": 0, "errors": 0}
    ...
    def is_whitelisted(self, sender, subject):
        ...
        return any(w.lower() in s for w in WHITELIST) or any(w.lower() in subject.lower() for w in IMPORTANT_SUBJECTS)
            ...
            for i, msg in enumerate(messages):
                ...
                try:
                    ...
                    if self.is_blacklisted(sender):
                        ...
                        domain = sender.split("@")[-1].split(">")[0] if "@" in sender else sender[:30]
...
def main():
    ...
    if not cleaner.authenticate(): return
