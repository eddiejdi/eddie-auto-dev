...
class EmailTrainer:
        ...
        try:
            ...
            try:
                ...
                self.collection = self.chroma_client.create_collection(
                    name="eddie_emails", metadata={"hnsw:space": "cosine"}
                )
    ...
    def extract_email_knowledge(self, email_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        # Criar ID único
        email_id = hashlib.md5(f"{sender_email}{subject}{date}".encode()).hexdigest()[
            :12
        ]
...
"""
        ...
        return {"id": f"email_{email_id}", "document": document, "metadata": metadata}
    ...
    def train_batch(
        self, emails: List[Dict[str, Any]], progress_callback=None
    ) -> Dict[str, Any]:
...
class EmailCleanerWithTraining:
    ...
    async def clean_with_training(
        self, max_emails: int = 100, dry_run: bool = True, train_important: bool = True
    ) -> Dict[str, Any]:
    ...
    if command in ["treinar_limpar", "train_clean", "limpar"]:
        if "confirmar" in args.lower() or "confirm" in args.lower():
            ...
            result = await cleaner.clean_with_training(
                max_emails=100, dry_run=False, train_important=True
            )
