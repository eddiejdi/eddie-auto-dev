...
if diretor:
    ...
    r = session.post(
        f"{BASE}/api/v1/models/create", headers=headers, json=update_payload
    )
    ...
    r = session.put(
        f"{BASE}/api/v1/models/update", headers=headers, json=update_payload
    )
    ...
    r = session.post(
        f"{BASE}/api/v1/models/{model_id}/update", headers=headers, json=update_payload
    )
...
for m in models:
    if m.get("id") == "diretor-eddie":
        ...
        if new_base == "qwen2.5-coder:7b":
            ...
            r = session.post(
                f"{BASE}/api/v1/models/create", headers=headers, json=new_model
            )
