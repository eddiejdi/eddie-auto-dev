...
try:
    ...
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "mcp", "httpx", "-q"]
    )
    from mcp.server import Server
    from mcp.server.stdio import stdio_server
    from mcp.types import (
...
class GitHubClient:
    ...
    async def list_repos(
        self, visibility: str = "all", sort: str = "updated", per_page: int = 30
    ) -> List[Dict[str, Any]]:
    ...
    async def list_commits(
        self, owner: str, repo: str, sha: str = None, per_page: int = 30
    ) -> List[Dict[str, Any]]:
    ...
    async def search_issues(
        self, query: str, sort: str = "created", per_page: int = 30
    ) -> Dict[str, Any]:
    ...
    async def create_gist(
        self, description: str, files: Dict[str, str], public: bool = True
    ) -> Dict[str, Any]:
        """Cria um novo gist"""
    ...
    async def list_notifications(
        self, all_notifications: bool = False, per_page: int = 30
    ) -> List[Dict[str, Any]]:
        """Lista notificações"""
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "sha": {
                        "type": "string",
                        "description": "Branch ou SHA para filtrar",
                    },
                    "per_page": {"type": "integer", "default": 30},
                },
                "required": ["owner", "repo"],
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "sha": {
                        "type": "string",
                        "description": "SHA do arquivo (para update)",
                    },
                    "branch": {"type": "string"},
                },
...
""",
        ...
        # Repositórios
            ...
            return [
                TextContent(
                    type="text", text=f"Repositórios ({len(result)}):\n{repos_summary}"
                )
            ]
            ...
            return [
                TextContent(
                    type="text", text=f"✅ Repositório criado: {result['html_url']}"
                )
            ]
            ...
            return [
                TextContent(
                    type="text",
                    text=f"✅ Repositório {arguments['owner']}/{arguments['repo']} deletado",
                )
            ]
        ...
        # Issues
            ...
            issues_summary = "\n".join(
                [f"#{i['number']} {i['title']} [{i['state']}]" for i in result[:20]]
            )
            return [
                TextContent(
                    type="text", text=f"Issues ({len(result)}):\n{issues_summary}"
                )
            ]
            ...
            return [
                TextContent(
                    type="text",
                    text=f"✅ Issue #{arguments['issue_number']} atualizada",
                )
            ]
            ...
            return [
                TextContent(
                    type="text", text=f"✅ Comentário adicionado: {result['html_url']}"
                )
            ]
        ...
        # Pull Requests
            ...
            return [
                TextContent(
                    type="text", text=f"Pull Requests ({len(result)}):\n{prs_summary}"
                )
            ]
            ...
            return [
                TextContent(
                    type="text", text=f"✅ PR #{arguments['pr_number']} merged!"
                )
            ]
        ...
        # Busca
            ...
            result = await github_client.search_code(
                query=arguments["query"], per_page=arguments.get("per_page", 30)
            )
            items = result.get("items", [])
            search_results = "\n".join(
                [f"• {i['repository']['full_name']}/{i['path']}" for i in items[:20]]
            )
            return [
                TextContent(
                    type="text",
        ...
        # GitHub Actions
            ...
            wf_list = "\n".join(
                [f"• {w['name']} (ID: {w['id']}) - {w['state']}" for w in workflows]
            )
            ...
            return [TextContent(type="text", text="✅ Workflow disparado!")]
        ...
        # Releases
            ...
            return [
                TextContent(
                    type="text", text=f"✅ Release criada: {result['html_url']}"
                )
            ]
        ...
        # Gists
            ...
            return [
                TextContent(type="text", text=f"✅ Gist criado: {result['html_url']}")
            ]
        ...
        # Outros
            ...
            notifs = "\n".join(
                [f"• [{n['reason']}] {n['subject']['title']}" for n in result[:20]]
            )
            ...
            return [
                TextContent(type="text", text=f"❌ Ferramenta desconhecida: {name}")
            ]
...
async def list_prompts() -> List[Prompt]:
    ...
    return [
        ...
        Prompt(
            name="list-my-repos", description="Listar meus repositórios", arguments=[]
        ),
        Prompt(
            name="create-issue",
            ...
            arguments=[
                PromptArgument(
                    name="repo", description="Repositório (owner/repo)", required=True
                ),
                PromptArgument(
                    name="title", description="Título da issue", required=True
                ),
                PromptArgument(
                    name="body", description="Descrição da issue", required=False
                ),
            ],
        ),
        Prompt(
    ...
    if name == "github-setup":
        ...
        return [
            PromptMessage(
                role="user",
                content=TextContent(
                    type="text", text="Liste meus repositórios do GitHub"
                ),
            )
        ]
