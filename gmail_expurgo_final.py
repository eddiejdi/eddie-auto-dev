...
def main():
    # Carregar token
    with open(
        "/home/homelab/myClaude/gmail_data/token.json", encoding="utf-8-sig"
    ) as f:
        t = json.load(f)
    ...
    for cat, dias in categorias:
        ...
        while True:
            try:
                ...
                total_limpo += len(ids)
                print(
                    f"{cat}: +{len(ids)} movidos (categoria: {cat_count}, total: {total_limpo})"
                )
    ...
    for cat, dias in categorias:
        ...
        r = (
            service.users()
            .messages()
            .list(userId="me", q=f"category:{cat} before:{data_limite}", maxResults=1)
            .execute()
        )
