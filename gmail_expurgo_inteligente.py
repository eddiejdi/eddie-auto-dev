...
class NotificationService:
    ...
    async def send_telegram(
        self, message: str, chat_id: int = None, parse_mode: str = "Markdown"
    ) -> Tuple[bool, str]:
    ...
    async def send_reminder(
        self, reminder: SmartReminder, channels: List[str] = None
    ) -> Dict[str, Any]:
...
"""
        ...
        return await self.notify(
            message, NotificationType.REMINDER, channels, reminder.priority
        )
...
Apenas JSON, sem explicação."""
            ...
            response = requests.post(
                ...
                json={"model": "eddie-assistant", "prompt": prompt, "stream": False},
                timeout=30,
            )
...
class ExpurgoInteligente:
    ...
    def _is_important_email(self, email_data: Dict[str, Any]) -> bool:
        ...
        content = (
            f"{email_data.get('subject', '')} {email_data.get('body', '')}".lower()
        )
        important_keywords = [
            "projeto",
            "reunião",
    ...
    async def process_emails_for_training(
        self, emails: List[Dict], notify_progress: bool = True
    ) -> Dict[str, Any]:
        ...
        return {"trained": trained, "reminders": reminders}

    async def send_reminders(
        self, reminders: List[SmartReminder], channels: List[str] = None
    ) -> int:
    ...
    async def run_expurgo(
        ...
        logger.info(
            f"   Notificações: {'Ativas' if send_notifications else 'Desativadas'}"
        )
        logger.info("=" * 50)
        ...
        for category, max_age_days in self.categories:
            date_limit = (datetime.now() - timedelta(days=max_age_days)).strftime(
                "%Y/%m/%d"
            )
            category_count = 0
            category_emails = []
            ...
            while True:
                try:
                    ...
                    logger.info(
                        f"  {category}: +{len(messages)} (total: {category_count})"
                    )
            ...
            if train_emails and category_emails:
                train_result = await self.process_emails_for_training(
                    category_emails, False
                )
            ...
            if category_count > 0:
                logger.info(
                    f"  >>> {category.upper()}: {category_count} emails processados"
                )
            else:
        ...
        if all_reminders and send_notifications:
            ...
            if high_priority_reminders:
                sent = await self.send_reminders(
                    high_priority_reminders, notification_channels
                )
...
"""
        ...
        for cat, data in results.items():
            ...
            report += (
                f"{emoji} {cat.upper()}: {data['deleted']} (>{data['max_age_days']}d)\n"
            )
        ...
        while True:
            try:
                await self.run_expurgo(
                    dry_run=False, train_emails=True, send_notifications=True
                )
            except Exception as e:
                logger.error(f"Erro no expurgo agendado: {e}")
                await self.notifier.notify(
                    f"❌ Erro no expurgo agendado: {e}", NotificationType.ERROR
                )
...
def main():
    ...
    parser.add_argument(
        "--execute", action="store_true", help="Executar de verdade (não dry run)"
    )
    parser.add_argument("--daemon", action="store_true", help="Executar como daemon")
    parser.add_argument(
        "--interval", type=int, default=24, help="Intervalo em horas (daemon)"
    )
    parser.add_argument(
        "--channels",
        nargs="+",
    ...
    parser.add_argument(
        "--no-notifications", action="store_true", help="Desabilitar notificações"
    )
    parser.add_argument(
        "--no-training", action="store_true", help="Desabilitar treinamento"
    )
