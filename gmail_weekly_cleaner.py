...
class EmailClassifier:
    ...
    def classify(cls, email: Email) -> Tuple[str, float]:
        ...
        # Verificar spam
        spam_score = sum(
            1 for kw in cls.SPAM_KEYWORDS if kw in subject_lower or kw in body_lower
        )
        if spam_score >= 2:
        ...
        # Verificar promoção
        promo_score = sum(
            1 for kw in cls.PROMO_KEYWORDS if kw in subject_lower or kw in body_lower
        )
...
class EmailTrainer:
    ...
    def __init__(self):
        ...
        self.collection = self.client.get_or_create_collection(
            name="emails", metadata={"description": "Emails importantes do usuário"}
        )
    ...
    def get_stats(self) -> Dict:
        ...
        return {"total_emails": self.collection.count(), "storage_path": CHROMA_PATH}
...
class GmailCleaner:
                ...
                if "parts" in msg_data["payload"]:
                    for part in msg_data["payload"]["parts"]:
                        if part["mimeType"] == "text/plain":
                            ...
                            break
                elif "body" in msg_data["payload"] and msg_data["payload"]["body"].get(
                    "data"
                ):
                    import base64
    ...
    def process_emails(self, dry_run: bool = False) -> Dict:
        ...
        for email in emails:
            try:
                ...
                if category == "spam":
                    ...
                    logger.info(
                        f"⭐ IMPORTANTE ({confidence:.0%}): {email.subject[:50]}"
                    )
        ...
        logger.info(
            f"💾 Total na base de conhecimento: {self.trainer.get_stats()['total_emails']}"
        )
        logger.info("=" * 60)
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Simula sem executar ações"
    )
    parser.add_argument(
        "--stats", action="store_true", help="Mostra apenas estatísticas"
    )
