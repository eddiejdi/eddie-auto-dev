...
def _find_entity(ha: HomeAssistantAPI, search: str, domain: str = None) -> Optional[Dict]:
    """
...
async def handle_homeassistant_command(
    ...
    if status["status"] != "connected":
        if status["status"] == "unauthorized":
            return "⚠️ Home Assistant: Token não configurado\nUse o painel web para configurar", False
    ...
    try:
        ...
        if command == "casa":
            ...
            relevant = ["light", "switch", "climate", "cover", "fan", "sensor", "binary_sensor"]
        ...
        # /luzes - Lista luzes
            ...
            return f"💡 *Luzes ({len(lights)}):*\n\n" + _format_device_list(lights, ha), True
        ...
        # /ligar - Liga dispositivo
            ...
            if not args:
                return "❌ Uso: `/ligar nome_do_dispositivo`\nEx: `/ligar luz_sala`", False
            
            device = _find_entity(ha, args)
            if not device:
                return f"❌ Dispositivo '{args}' não encontrado\nUse `/dispositivos` para ver a lista", False
