...
def main():
    ...
    function_file = os.path.join(
        os.path.dirname(__file__), "openwebui_agent_coordinator_function.py"
    )
    ...
    if function_exists:
        ...
        r = requests.post(
            f"{base_url}/api/v1/functions/create", headers=headers, json=function_data
        )
    ...
    if r.status_code in [200, 201]:
        ...
        r = requests.post(
            f"{base_url}/api/v1/functions/id/{function_id}/toggle", headers=headers
        )
        if r.status_code == 200:
