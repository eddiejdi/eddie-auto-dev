...
def load_geofences_from_config() -> Dict[str, GeoFence]:
    ...
    if not geofences:
        print("⚠️ Usando geofences padrão - configure config.json com suas coordenadas!")
        geofences = {
            "casa": GeoFence(
                name="Casa",
...
class LocationDatabase:
    ...
    def save_event(self, event_type: str, geofence: str, lat: float, lon: float, details: Dict = None):
...
async def process_location(data: Dict, background_tasks: BackgroundTasks):
    ...
    return {
        ...
        "geofence_events": [
            {"type": e["type"].value, "fence": e["fence"].name}
