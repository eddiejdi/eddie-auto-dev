...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        AUTOCOINBOT_API: str = Field(
            default="http://192.168.15.2:8510", description="URL da API do AutoCoinBot"
        )
        ...
        DIRECTOR_MODEL: str = Field(
            default="qwen2.5-coder:7b", description="Modelo do Diretor"
        )
        TELEGRAM_NOTIFY: bool = Field(
            default=True, description="Enviar notificações via Telegram"
        )
    ...
    async def pipe(
            ...
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {
                        "description": "👔 Diretor Eddie processando...",
                        "done": False,
                    },
                }
            )
        ...
        try:
            ...
            if last_message.startswith("/"):
                ...
                result = await self._director_response(
                    last_message, messages, __event_emitter__
                )
            ...
            await emitter(
                {
                    "type": "status",
                    "data": {
                        "description": "📊 Buscando dados do AutoCoinBot...",
                        "done": False,
                    },
                }
            )
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                ...
                if r.status_code == 200:
                    ...
                    # Preço atual
                    report.append(
                        f"**💰 Preço Atual:** {data.get('price_formatted', 'N/A')}"
                    )
                    ...
                    report.append(
                        f"  • Winning Trades: {perf.get('winning_trades', 0)}"
                    )
                    ...
                    if total_pnl >= 20:
                        ...
                        report.append(
                            "  • ❌ ALERTA: Prejuízo > 15% - Reciclagem obrigatória"
                        )
                    elif total_pnl <= -10:
                        report.append(
                            "  • 🔴 ALERTA: Prejuízo > 10% - Operações suspensas"
                        )
                    elif total_pnl <= -5:
                        report.append("  • 🔶 ALERTA: Prejuízo > 5% - Trading pausado")
                    elif total_pnl <= -2:
                        report.append(
                            "  • ⚠️ ALERTA: Prejuízo > 2% - Revisar estratégia"
                        )
                    else:
            ...
            await emitter(
                {
                    "type": "status",
                    "data": {
                        "description": "🧠 Analisando instrução...",
                        "done": False,
                    },
                }
            )
...
Se for uma tarefa complexa, quebre em etapas seguindo o pipeline."""
                ...
                await emitter(
                    {
                        "type": "status",
                        "data": {
                            "description": f"{phase_name}: {status_msg}",
                            "done": False,
                        },
                    }
                )
    ...
    async def _delegate_task(self, args: str, emitter) -> str:
        ...
        if agent_name not in all_agents:
            return (
                f"❌ Agent `{agent_name}` não encontrado.\n\nAgents disponíveis:\n"
            ...
            await emitter(
                {
                    "type": "status",
                    "data": {
                        "description": f"📤 Delegando para {agent_name}...",
                        "done": False,
                    },
                }
            )
