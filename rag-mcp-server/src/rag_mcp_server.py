...
async def rag_get_context(args: dict) -> list[TextContent]:
    ...
    response = await http_client.get(
        f"{RAG_API_BASE}/rag/context",
        params={"query": query, "n_results": n_results}
    )
    
    if response.status_code != 200:
        return [TextContent(type="text", text=f"Erro ao obter contexto: {response.status_code}")]
    
    data = response.json()
    return [TextContent(type="text", text=f"## 📚 Contexto RAG\n\n{json.dumps(data, indent=2, ensure_ascii=False)}")]
...
async def rag_index_document(args: dict) -> list[TextContent]:
    ...
    if response.status_code == 200:
        data = response.json()
        return [TextContent(type="text", text=f"✅ Documento indexado!\n- ID: {doc_id}\n- Collection: {args.get('collection', 'default')}\n- Mensagem: {data.get('message', 'OK')}")]
    else:
        return [TextContent(type="text", text=f"❌ Erro ao indexar: {response.status_code} - {response.text}")]
...
async def rag_stats() -> list[TextContent]:
    ...
    if response.status_code != 200:
        return [TextContent(type="text", text=f"Erro ao obter stats: {response.status_code}")]
    ...
    output += f"- **Taxa Feedback Positivo:** {data.get('positive_feedback_rate', 0):.1%}\n"
...
async def rag_index_project(args: dict) -> list[TextContent]:
    ...
    if response.status_code == 200:
        data = response.json()
        return [TextContent(type="text", text=f"✅ Projeto indexado!\n{json.dumps(data, indent=2, ensure_ascii=False)}")]
    else:
        return [TextContent(type="text", text=f"❌ Erro: {response.status_code} - {response.text}")]
...
async def rag_trigger_learning() -> list[TextContent]:
    ...
    if response.status_code == 200:
        data = response.json()
        return [TextContent(type="text", text=f"✅ Aprendizado disparado!\n{json.dumps(data, indent=2, ensure_ascii=False)}")]
    else:
        return [TextContent(type="text", text=f"❌ Erro: {response.status_code}")]
