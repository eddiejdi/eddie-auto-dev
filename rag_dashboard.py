...
def get_ha_token():
    try:
        ...
        config_path = os.path.join(
            os.path.dirname(__file__), "homeassistant_integration", "config.json"
        )
...
if page == "🏠 Visão Geral":
    ...
    if stats:
        ...
        with col3:
            st.metric(label="📁 Collections", value=len(stats.get("collections", [])))
        ...
        for i, coll in enumerate(collections):
            with cols[i]:
                icon = (
                    ...
                    else "📚" if coll == "chat_history" else "📁"
                )
                st.info(f"{icon} **{coll}**")
    ...
    if ha_status["status"] == "offline":
        ...
        **Para configurar o Home Assistant:**
            ...
            if new_token:
                try:
                    ...
                    with open(config_path, "w") as f:
                        json.dump(
                            {"url": "http://localhost:8123", "token": new_token},
                            f,
                            indent=2,
                        )
        ...
        st.success(
            f"✅ Conectado ao Home Assistant - {config.get('location_name', 'Casa')}"
        )
        ...
        binary_sensors = [
            s for s in states if s.get("entity_id", "").startswith("binary_sensor.")
        ]
        ...
        with col2:
            ...
            st.metric(
                "🔌 Tomadas", f"{switches_on}/{len(switches)}", f"{switches_on} ligadas"
            )
        with col3:
        ...
        if lights:
            ...
            for i, light in enumerate(lights):
                with cols[i % 4]:
                    ...
                    if st.button(
                        f"{'Desligar' if is_on else 'Ligar'}", key=f"btn_{entity_id}"
                    ):
                        if is_on:
        ...
        if switches:
            ...
            for i, switch in enumerate(switches):
                with cols[i % 4]:
                    ...
                    name = switch.get("attributes", {}).get(
                        "friendly_name", entity_id.split(".")[-1]
                    )
        ...
        if sensors or binary_sensors:
            ...
            with st.expander(f"📊 Sensores ({len(sensors) + len(binary_sensors)})"):
                for sensor in (sensors + binary_sensors)[:20]:
                    ...
                    name = sensor.get("attributes", {}).get(
                        "friendly_name", entity_id.split(".")[-1]
                    )
    ...
    if stats:
        ...
        for coll in collections:
            with st.expander(f"📁 {coll}", expanded=(coll == "bitcoin_knowledge")):
                ...
                if coll == "bitcoin_knowledge":
                    st.success(
                        "🪙 Esta collection contém conhecimento especializado em Bitcoin!"
                    )
                    st.markdown("""
                    **Tópicos cobertos:**
    ...
    with col1:
        query = st.text_input(
            "Digite sua pergunta:", placeholder="Ex: O que é Bitcoin?"
        )
    with col2:
                ...
                if results:
                    for i, r in enumerate(results):
                        with st.expander(f"📄 Resultado {i + 1}", expanded=(i == 0)):
                            ...
                            if metadata:
                                st.caption(
                                    f"📌 Tópico: {metadata.get('topic', 'N/A')} | Fonte: {metadata.get('source', 'N/A')}"
                                )
                else:
    ...
    with col1:
        prompt = st.text_area(
            "Digite seu prompt:",
            placeholder="Ex: Explique o que é Bitcoin em 3 frases.",
            height=100,
        )
    with col2:
    ...
    if st.button("🚀 Iniciar Benchmark Completo", type="primary"):
        ...
        results = {"rag_searches": [], "ollama_inferences": []}
        ...
        for i, p in enumerate(test_prompts):
            ...
            progress.progress(
                (len(test_queries) + i + 1) / (len(test_queries) + 3) * 100 / 100
            )
        ...
        with col1:
            ...
            st.metric(
                "Latência Mínima",
                f"{min(rag_latencies):.0f}ms" if rag_latencies else "N/A",
            )
            st.metric(
                "Latência Máxima",
                f"{max(rag_latencies):.0f}ms" if rag_latencies else "N/A",
            )
        ...
        with col2:
            ...
            st.metric(
                "Latência Mínima",
                f"{min(ollama_latencies) / 1000:.1f}s" if ollama_latencies else "N/A",
            )
            st.metric(
                "Latência Máxima",
                f"{max(ollama_latencies) / 1000:.1f}s" if ollama_latencies else "N/A",
            )
