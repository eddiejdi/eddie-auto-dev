#!/usr/bin/env bash
# Ativa perfis SOL-USDT live na conta TRADE master (kucoin/homelab).
...
# Docker prometheus no homelab monta este arquivo (não /etc/prometheus/).
...
declare -a PROFILES=(
  "SOL_USDT_shadow:9108:8518"
  "SOL_USDT_conservative:9104:8516"
  "SOL_USDT_aggressive:9106:8517"
)

echo "=== Ativação SOL-USDT (master TRADE, live) ==="
...
for cfg in config_SOL_USDT_shadow.json config_SOL_USDT_conservative.json config_SOL_USDT_aggressive.json; do
...
if [[ -f "${REPO_PROMETHEUS}" ]]; then
  if grep -q "crypto-exporter-sol_usdt_shadow" "${REPO_PROMETHEUS}"; then
...
echo "=== SOL-USDT ativo. Garanta ~\$150 USDT na TRADE master (\$50 × 3 perfis). ==="