...
sync_runtime_file() {
  ...
  require_file "${src}"
  sudo install -d -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 0755 "$(dirname "${dst}")"
  sudo install -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 0644 "${src}" "${dst}"
}
...
write_trading_database_env() {
  ...
  sudo install -d -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 0750 "${ENVFILES_DIR}"
  sudo install -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 0640 "${tmp_env}" "${TRADING_DB_ENV}"
  rm -f "${tmp_env}"
}
...
ensure_trading_venv() {
  ...
  if [[ ! -x "${TRADING_VENV}/bin/python" ]]; then
    ...
    sudo install -d -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 0755 "$(dirname "${TRADING_VENV}")"
    sudo python3 -m venv "${TRADING_VENV}"
    sudo chown -R "${SERVICE_USER}:${SERVICE_USER}" "${TRADING_VENV}"
  fi
...
sudo install -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 0644 "${CONSERVATIVE_SRC}" "${CONSERVATIVE_DST}"
sudo install -o "${SERVICE_USER}" -g "${SERVICE_USER}" -m 0644 "${AGGRESSIVE_SRC}" "${AGGRESSIVE_DST}"
