...
def ensure_dns_route(cloudflared_bin: str, tunnel_name: str, hostname: str) -> bool:
    """Cria a rota DNS do hostname para o tunnel quando necessário."""
    result = run_command([cloudflared_bin, "tunnel", "route", "dns", tunnel_name, hostname])
...
def ensure_dns_routes(config_path: Path, cloudflared_bin: str) -> bool:
    ...
    return all(ensure_dns_route(cloudflared_bin, tunnel_name, hostname) for hostname in REQUIRED_HOSTNAMES)
...
def main() -> int:
    ...
    if ensure_dns_routes(config_path, args.cloudflared_bin):
        return 0
