...
if CFG.exists():
    ...
    try:
        ...
        m = re.search(r"SERVER\s*=\s*ServerConfig\([^\)]*ip\s*=\s*['\"]([0-9a-fA-F:\.]+)['\"]", src)
        if m:
            SERVER_IP = m.group(1)
        else:
    ...
    OVERRIDES = {}
    overrides_path = os.path.join(os.path.dirname(__file__), '..', 'docs', 'public_url_overrides.json')
    try:
...
for name, url in sorted(URLS.items()):
    try:
        ...
        extra = f' (<a href="{link}" target="_blank">open</a>)' if str(url).startswith(('http://','https://')) else ''
        lines.append(f'<li><strong>{display}</strong>: <a href="{link}" target="_blank">{link}</a>{extra}</li>')
    except Exception:
        continue
...
if PORTS:
    ...
    for svc, p in sorted(PORTS.items()):
        try:
            ...
            lines.append(f'<li>{html.escape(str(svc))}: <a href="{html.escape(url)}" target="_blank">{html.escape(url)}</a></li>')
        except Exception:
            continue
    lines.append('</ul>')
