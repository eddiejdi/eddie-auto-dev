...
Responda APENAS com o JSON, sem explicações adicionais."""
    ...
    def execute_action(self, intent: ParsedIntent) -> dict:
        ...
        actions = {
            ...
            GitHubAction.GET_REPO: lambda: self.github.get_repo(
                p["owner"], p["repo"]
            ),
            ...
            GitHubAction.GET_USER: lambda: self.github.get_user(
                p.get("username")
            ),
...
Responda em português brasileiro:"""
    ...
    def process(self, user_input: str) -> str:
        ...
        print(f"📋 Ação identificada: {intent.action.value} (confiança: {intent.confidence:.0%})")
...
def interactive_mode():
    ...
    print(f"🔑 GitHub Token: {'✅ Configurado' if GITHUB_TOKEN else '❌ Não configurado'}")
    print("-" * 60)
