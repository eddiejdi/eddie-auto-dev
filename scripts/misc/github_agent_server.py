...
Responda APENAS com JSON válido."""
    ...
    def execute(self, intent):
        ...
        actions = {
            ...
            "list_issues": lambda: self.github.list_issues(p["owner"], p["repo"], p.get("state", "open")),
            "create_issue": lambda: self.github.create_issue(p["owner"], p["repo"], p["title"], p.get("body", "")),
            "list_prs": lambda: self.github.list_prs(p["owner"], p["repo"], p.get("state", "open")),
...
def github_callback():
    ...
    if not token:
        flash(f"Erro ao obter token: {data.get('error_description', 'Desconhecido')}", "error")
        return redirect(url_for("index"))
    ...
    flash(f"Login realizado com sucesso! Bem-vindo, {user_info.get('login', 'usuário')}!", "success")
...
def token_login():
    ...
    set_github_token(token, user_info)
    flash(f"Token configurado! Bem-vindo, {user_info.get('login', 'usuário')}!", "success")
