#!/usr/bin/env python3
"""
Processador de Lotes - Refatoração Incremental com Ollama Remoto
Salva resultados incrementalmente + cache + resumo de progresso
"""
...
async def analyze_file(file_path: Path, gpu_url: str, gpu_id: int) -> dict:
    """Analisa arquivo com estratégia otimizada."""
    ...
    try:
        ...
        logger.info(f"[GPU{gpu_id}] ✓ {file_path.name} ({result['shared_count']} refs SHARED)")
        return result
...
async def process_lote(lote_num: int, files: list[Path]) -> dict:
    ...
    # Processar em paralelo (GPU0 + GPU1)
    tasks = []
    ...
    logger.info(f"📊 Lote {lote_num} resumo:")
    logger.info(f"   Processados: {len(results)}")
    logger.info(f"   Sucesso: {sucesso}/{len(results)}")
    logger.info(f"   Refs SHARED: {shared_total}")
    logger.info(f"   Salvos em: analysis_results/lote_{lote_num:02d}.json")
...
async def process_lote_1():
    """LOTE 1: btc_trading_agent + shared_tray_agent."""
    
    # Coletar arquivos
    base_path = Path("/home/edenilson/shared-auto-dev")
    
    files = []
    for comp in ["btc_trading_agent", "shared_tray_agent"]:
    ...
    logger.info(f"Total: {len(files)} arquivos")
    ...
    # Salvar resumo geral
    summary_file = RESULTS_DIR / "LOTE1_RESUMO.json"
