...
with sync_playwright() as p:
    ...
    try:
        ...
        submit_btn = None
        try:
            page.wait_for_selector('input[name="email"], input[type="email"], input[placeholder*="Email"], input[placeholder*="email"]', timeout=10000)
            form_email = page.query_selector('input[name="email"]') or page.query_selector('input[type="email"]') or page.query_selector('input[placeholder*="Email"]') or page.query_selector('input[placeholder*="email"]')
            form_pass = page.query_selector('input[name="password"]') or page.query_selector('input[type="password"]')
        ...
        if form_email and submit_btn and email and password:
            ...
            possible_pw = [
                ...
                'input[id*="password"]', 'input[placeholder*="Password"]', 'input[placeholder*="password"]'
            ]
            pw_selector = None
            ...
            for i in range(20):
                time.sleep(0.5)
                token = page.evaluate("() => window.localStorage.getItem('token') || window.localStorage.getItem('access_token')")
                if token:
        ...
        if not token:
            import requests
            try:
                r = requests.post(base + '/api/v1/auths/signin', json={'email': email, 'password': password}, timeout=10)
        ...
        try:
            ...
            r = requests.post(base + '/api/v1/chats/new', headers=headers, json={'chat':{'title':'playwright-check'}}, timeout=10)
            log('create_chat_status=' + str(r.status_code) + ' body=' + (r.text[:200] if r.text else ''))
            r2 = requests.get(base + '/api/v1/chats/list', headers=headers, timeout=10)
            log('list_chats_status=' + str(r2.status_code) + ' body_len=' + str(len(r2.text)))
        except Exception as e:
