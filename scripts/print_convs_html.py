...
def build_html(convs):
    ...
    for i, c in enumerate(convs):
        ...
        if cid:
            try:
                mr = requests.get(f"{API}/interceptor/conversations/{cid}/messages", timeout=3)
                mr.raise_for_status()
        ...
        parts.append(f"<div style=\"background:{color};padding:10px;margin:6px 0;border-radius:6px;\">{text}</div>")
