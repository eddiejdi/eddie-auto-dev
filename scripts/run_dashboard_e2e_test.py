...
def kill_existing():
    subprocess.run(
        ["pkill", "-f", APP], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
...
def wait_for_http(
    url="https://heights-treasure-auto-phones.trycloudflare.com/", timeout=60
):
...
def find_created_conv_in_db(db_path="agent_data/interceptor_data/conversations.db"):
    try:
        ...
        cur.execute(
            "SELECT id, started_at FROM conversations WHERE id LIKE 'ui_test_conv_%' ORDER BY started_at DESC LIMIT 1"
        )
        row = cur.fetchone()
        conn.close()
        if row:
...
def main():
    ...
    try:
        ...
        if "Interceptor de Conversas entre Agentes" in content:
            ...
            print(
                "Warning: expected title not found in HTML. Content length:",
                len(content),
            )
        ...
        try:
            ...
            req = Request(
                "http://127.0.0.1:8765/create_test_conv",
                method="POST",
        ...
        if conv_line:
            ...
            print(
                "Auto-create marker not found in log; checking DB for ui_test_conv_..."
            )
            ...
            if not db_row and conv_id:
                ...
                cur = conn.cursor()
                cur.execute(
                    "SELECT id, started_at FROM conversations WHERE id = ? LIMIT 1",
                    (conv_id,),
                )
                row = cur.fetchone()
                conn.close()
            ...
            if db_row:
                ...
                print(
                    "Auto-create conversation marker not found in log or DB — check streamlit runtime logs:",
                    LOG,
                )
                ...
                try:
                    ...
                    pass
                raise RuntimeError(
                    "Auto-created conversation not observed in streamlit log or DB"
                )
