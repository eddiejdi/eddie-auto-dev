...
`;
  ...
  async function handleAIGenerate(shouldRun) {
    ...
    updateStatus('Gerando com IA...');
    output.textContent = '🧠 Gerando código com IA (stream)...\n';

    try {
      let code = '';
      try {
        code = await generateCodeWithAIStream(promptEl.value.trim(), (fullCode) => {
          if (editor) {
        ...
        code = await generateCodeWithAI(promptEl.value.trim());
        if (editor) {
      ...
      updateStatus('Código gerado');
      output.textContent = '✅ Código gerado pela IA.';
      ...
      output.textContent = `❌ Erro ao gerar código: ${error.message}`;
  ...
  function init() {
    ...
    const aiGenerateRunBtn = document.getElementById('aiGenerateRun');
    if (aiGenerateRunBtn) {
      aiGenerateRunBtn.addEventListener('click', () => handleAIGenerate(true));
    }

    const aiGenerateFilesBtn = document.getElementById('aiGenerateFiles');
    if (aiGenerateFilesBtn) {
      aiGenerateFilesBtn.addEventListener('click', () => handleAIGenerateFiles(false));
    }
