...
def setup_tuya_credentials():
    ...
    device_id = input(
        "\nDevice ID (qualquer dispositivo, ou Enter para pular): "
    ).strip()
...
async def test_connection():
    ...
    try:
        ...
        cloud = tinytuya.Cloud(
            ...
            apiDeviceID=(
                tuya_config.get("device_id")
                if tuya_config.get("device_id") != "auto"
                else None
            ),
        )
    ...
    try:
        if command == "on":
            result = cloud.sendcommand(
                device_id, {"commands": [{"code": "switch", "value": True}]}
            )
        elif command == "off":
            result = cloud.sendcommand(
                device_id, {"commands": [{"code": "switch", "value": False}]}
            )
        elif command == "speed" or command == "fan_speed":
            # Para ventiladores - speed geralmente vai de 1-100 ou níveis específicos
            result = cloud.sendcommand(
                device_id, {"commands": [{"code": "fan_speed", "value": int(value)}]}
            )
        elif command == "brightness" or command == "dim":
            result = cloud.sendcommand(
                device_id, {"commands": [{"code": "bright_value", "value": int(value)}]}
            )
        elif command == "max":
            ...
            if "fan" in category or "ventilador" in device.get("name", "").lower():
                result = cloud.sendcommand(
                    device_id, {"commands": [{"code": "fan_speed", "value": 100}]}
                )
            else:
                result = cloud.sendcommand(
                    device_id, {"commands": [{"code": "bright_value", "value": 100}]}
                )
        elif command == "status":
            ...
            # Comando genérico
            result = cloud.sendcommand(
                device_id, {"commands": [{"code": command, "value": value}]}
            )
