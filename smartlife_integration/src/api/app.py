...
def create_app(config: dict = None, service: SmartLifeService = None) -> FastAPI:
    ...
    app.include_router(
        automations.router, prefix="/api/automations", tags=["Automations"]
    )
    ...
    @app.get("/", tags=["Health"])
    async def root():
        return {"service": "SmartLife API", "version": "1.0.0", "status": "running"}
    ...
    async def health():
        ...
        if smartlife_service:
            ...
            status["automations_enabled"] = (
                smartlife_service.automation_engine is not None
            )
