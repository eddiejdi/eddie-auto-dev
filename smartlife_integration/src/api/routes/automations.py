...
class TriggerConfig(BaseModel):
    ...
    type: str = Field(
        ..., description="Tipo: time, cron, device_state, sensor, sunrise, sunset"
    )
    ...
    device_id: Optional[str] = Field(
        None, description="ID do dispositivo para trigger de estado"
    )
...
class ActionConfig(BaseModel):
    ...
    device_id: str = Field(
        ..., description="ID do dispositivo ou 'all_lights', 'all_devices'"
    )
...
class ConditionConfig(BaseModel):
    ...
    device_id: Optional[str] = Field(
        None, description="ID do dispositivo para condição"
    )
...
async def create_automation(automation: AutomationCreate):
    ...
    result = await service.automation_engine.create(
        ...
        conditions=(
            [c.model_dump() for c in automation.conditions]
...
async def list_trigger_types():
    ...
    return {
        "time": {"description": "Horário específico", "params": ["time (HH:MM)"]},
        "cron": {"description": "Expressão cron", "params": ["cron (0 8 * * *)"]},
        "sunrise": {
...
async def list_automation_templates():
    ...
    return [
        ...
        },
        {
            ...
            "trigger": {
                "type": "device_state",
                "device_id": "sensor_porta",
                "state": {"open": True},
            },
            "conditions": [
                {"type": "time_range", "start_time": "18:00", "end_time": "23:00"}
            ],
            "actions": [
