...
DEVICE_TYPES = {
    "switch": {"name": "Tomada/Interruptor", "icon": "🔌", "commands": ["on", "off", "toggle"]},
    "light": {"name": "Lâmpada", "icon": "💡", "commands": ["on", "off", "toggle", "dim", "color"]},
    ...
    "airconditioner": {"name": "Ar Condicionado", "icon": "❄️", "commands": ["on", "off", "temp", "mode"]},
    "heater": {"name": "Aquecedor", "icon": "🔥", "commands": ["on", "off", "temp"]},
    "fan": {"name": "Ventilador", "icon": "🌀", "commands": ["on", "off", "speed"]},
    "humidifier": {"name": "Umidificador", "icon": "💧", "commands": ["on", "off", "level"]},
    "cover": {"name": "Cortina/Persiana", "icon": "🪟", "commands": ["open", "close", "stop", "position"]},
    ...
    "robot_vacuum": {"name": "Robô Aspirador", "icon": "🤖", "commands": ["start", "stop", "dock"]},
}
...
class DeviceManager:
    ...
    def _build_tuya_commands(
        ...
        if command in ["on", "off"]:
            return [{"code": "switch_led" if device_type == "light" else "switch_1", 
                    "value": command == "on"}]
        ...
        if command == "color":
            hex_color = COLOR_MAP.get(value.lower(), value) if isinstance(value, str) else value
