...
from ..database.repository import DeviceRepository, AutomationRepository
...
class SmartLifeService:
    ...
    async def get_devices(self, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if user_id and self.user_manager:
            ...
            if user and user.role != "admin":
                allowed_ids = await self.user_manager.get_user_device_permissions(user_id)
    ...
    async def control_device(
        ...
        if user_id:
            ...
            if not has_permission:
                return {"success": False, "error": "Sem permissão para controlar este dispositivo"}
    ...
    async def turn_on(self, device_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        """Liga um dispositivo."""
        return await self.control_device(device_id, "on", user_id=user_id)
    
    async def turn_off(self, device_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        """Desliga um dispositivo."""
        return await self.control_device(device_id, "off", user_id=user_id)
    
    async def toggle(self, device_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        devices = await self.device_manager.get_all_devices() if self.device_manager else []
        ...
        return {
            ...
            "automations": {
                "total": len(await self.automation_engine.get_all()) if self.automation_engine else 0,
                "active": len([a for a in await self.automation_engine.get_all() if a.get("is_active")]) if self.automation_engine else 0
            },
            "connections": {
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "components": {
                ...
                "tuya_local": self.tuya_local.is_connected if self.tuya_local else False,
                "tuya_cloud": self.tuya_cloud.is_connected if self.tuya_cloud else False
            }
        }
