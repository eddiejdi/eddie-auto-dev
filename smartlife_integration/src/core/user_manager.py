...
class UserManager:
    ...
    async def create_user(
        ...
        if telegram_id:
            ...
            if existing:
                return {"success": False, "error": "Usuário já existe", "user": existing}
    ...
    async def delete_user(self, user_id: str) -> Dict[str, Any]:
        ...
        if user.get("role") == UserRole.ADMIN:
            admin_count = sum(1 for u in self._users.values() if u.get("role") == UserRole.ADMIN)
            if admin_count <= 1:
                return {"success": False, "error": "Não é possível remover o último admin"}
    ...
    async def get_user_device_permissions(self, user_id: str) -> List[str]:
        ...
        return [
            device_id for device_id, perm in permissions.items()
