...
class DeviceRepository:
    ...
    async def get_by_room(self, room: str) -> List[Device]:
        ...
        result = await self.session.execute(select(Device).where(Device.room == room))
        return list(result.scalars().all())
...
class UserRepository:
    ...
    async def set_permission(
        ...
        result = await self.session.execute(
            select(Permission).where(
                and_(Permission.user_id == user_id, Permission.device_id == device_id)
            )
        )
    ...
    async def check_permission(
        self, user_id: int, device_id: str, action: str = "view"
    ) -> bool:
...
class AutomationRepository:
    ...
    async def update(
        self, automation_id: int, data: Dict[str, Any]
    ) -> Optional[Automation]:
    ...
    async def record_execution(
        self, automation_id: int, error: Optional[str] = None
    ) -> bool:
...
class SceneRepository:
    ...
    async def get_by_id(self, scene_id: int) -> Optional[Scene]:
        """Busca cena por ID."""
        result = await self.session.execute(select(Scene).where(Scene.id == scene_id))
        return result.scalar_one_or_none()
    ...
    async def delete(self, scene_id: int) -> bool:
        """Remove uma cena."""
        result = await self.session.execute(delete(Scene).where(Scene.id == scene_id))
        await self.session.commit()
        return result.rowcount > 0
...
class EventRepository:
    ...
    async def get_errors(self, hours: int = 24) -> List[Event]:
        ...
        result = await self.session.execute(
            select(Event)
            .where(and_(Event.severity == "error", Event.created_at >= since))
            .order_by(desc(Event.created_at))
        )
        return list(result.scalars().all())
