...
class TuyaLocalClient:
    ...
    def __init__(
        self, devices_file: str = "config/devices.json", scan_interval: int = 60
    ):
    ...
    async def _load_devices_file(self) -> None:
        ...
        if not self.devices_file.exists():
            logger.warning(
                f"Arquivo {self.devices_file} não encontrado. Execute: python -m tinytuya wizard"
            )
            return
        ...
        try:
            ...
            logger.info(
                f"Carregados {len(self._devices)} dispositivos de {self.devices_file}"
            )
    ...
    async def get_status(
        self, device_id: str, ip: str, local_key: str, version: str = "3.3"
    ...
    async def send_command(
        ...
        try:
            ...
            def _send_command():
                ...
                if command in ["dim", "color"]:
                    d = tinytuya.BulbDevice(
                        device_id, ip, local_key, version=float(version)
                    )
                ...
                if command == "on":
                    ...
                    status = d.status()
                    if status and "dps" in status:
                        is_on = status["dps"].get("1", False) or status["dps"].get(
                            "20", False
                        )
            ...
            if result is None or (
                isinstance(result, dict) and "Error" not in str(result)
            ):
