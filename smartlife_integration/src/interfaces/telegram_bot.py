...
class SmartLifeTelegramBot:
    ...
    def __init__(self, token: str, smartlife_service, admin_ids: List[int] = None):
        self.token = token
    ...
    async def cmd_devices(
        ...
        keyboard.append(
            [InlineKeyboardButton("🔄 Atualizar", callback_data="sl_devices")]
        )
    ...
    async def cmd_dim(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        result = await self.smartlife.control_device(
            device_id=device_name, command="dim", value=brightness, user_id=user["id"]
        )

        if result.get("success"):
            await update.message.reply_text(
                f"🔆 Brilho de *{device_name}* ajustado para {brightness}%",
                parse_mode="Markdown",
            )
        else:
            await update.message.reply_text(
    ...
    async def cmd_color(
        ...
        result = await self.smartlife.control_device(
            device_id=device_name, command="color", value=color, user_id=user["id"]
        )

        if result.get("success"):
            await update.message.reply_text(
                f"🎨 Cor de *{device_name}* alterada para {color}",
                parse_mode="Markdown",
            )
        else:
            await update.message.reply_text(
    ...
    async def _control_device(
        ...
        result = await self.smartlife.control_device(
            device_id=device_name, command=command, user_id=user["id"]
        )
    ...
    async def cmd_scenes(
        ...
        for scene in scenes:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        f"🎬 {scene['name']}", callback_data=f"sl_scene_{scene['id']}"
                    )
                ]
            )
    ...
    async def cmd_scene(
        ...
        if not args:
            await update.message.reply_text(
                "Uso: `/scene <nome>`\nExemplo: `/scene cinema`", parse_mode="Markdown"
            )
            return
        ...
        if result.get("success"):
            await update.message.reply_text(
                f"🎬 Cena *{scene_name}* executada!", parse_mode="Markdown"
            )
        else:
            await update.message.reply_text(
                f"❌ Erro: {result.get('error', 'Desconhecido')}"
    ...
    async def cmd_adduser(
        ...
        if result.get("success"):
            await update.message.reply_text(
                f"✅ Usuário *{name}* adicionado!", parse_mode="Markdown"
            )
        else:
            await update.message.reply_text(
                f"❌ Erro: {result.get('error', 'Desconhecido')}"
    ...
    async def cmd_deluser(
        ...
        if not args:
            await update.message.reply_text(
                "Uso: `/deluser <telegram_id>`", parse_mode="Markdown"
            )
            return
    ...
    async def cmd_refresh(
        ...
        if not await self._check_admin(update):
            await update.message.reply_text(
                "❌ Apenas admins podem atualizar dispositivos."
            )
            return
