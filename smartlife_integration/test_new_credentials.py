...
def get_devices(token):
    ...
    if users_result.get('success') and users_result.get('result'):
        ...
        for user in users_result['result']:
            ...
            devices_resp = requests.get(f'{BASE_URL}/v1.0/users/{uid}/devices', headers=headers2)
        ...
        return {"success": False, "error": users_result.get('msg', 'No users'), "response": users_result}
...
if token_result.get('success'):
    ...
    if devices_result.get('success'):
        ...
        for i, device in enumerate(devices, 1):
            ...
            print(f"      Local Key: {local_key[:8]}..." if local_key else "      Local Key: N/A")
            print(f"      IP: {ip}")
            print()
        ...
        print(f"   Resposta: {json.dumps(devices_result.get('response', {}), indent=2)}")
        print()
