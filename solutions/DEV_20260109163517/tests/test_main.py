...
def test_build_docker_image(mock_run):
    ...
    mock_run.assert_called_once_with(["docker", "build", "-t", image_name, "."], check=True)
...
def test_deploy_with_kubectl(mock_run):
    ...
    mock_run.assert_called_once_with(["kubectl", "run", image_name, "--image", image_name], check=True)
...
def test_check_app_accessibility(mock_run):
    ...
    mock_run.assert_called_once_with(["curl", "-s", url], capture_output=True, text=True)
...
def test_check_api_endpoint(mock_run):
    ...
    mock_run.assert_called_once_with(["curl", "-s", f"{url}/api/v1/health"], capture_output=True, text=True)
