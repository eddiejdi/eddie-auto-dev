...
def get_weather_forecast(city):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Erro ao obter previsão do tempo. Código de status: {response.status_code}"
            )
...
def test_get_weather_forecast_valid_city():
    ...
    assert (
        "Previsão para São Paulo:" in forecast
    ), "A previsão não contém o título correto."
...
def test_get_weather_forecast_exception_handling():
    ...
    assert (
        str(e.value) == "Erro ao obter previsão do tempo. Código de status: 404"
    ), "Deveria lançar um erro para uma cidade inválida."
