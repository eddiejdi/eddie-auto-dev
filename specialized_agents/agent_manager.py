...
from .base_agent import SpecializedAgent, TaskStatus
from .language_agents import create_agent, AGENT_CLASSES
...
from .requirements_analyst import get_requirements_analyst
...
class AgentManager:
    ...
    async def initialize(self):
        ...
        if not self.docker.is_available():
            print(
                "[Warning] Docker não disponível. Algumas funcionalidades serão limitadas."
            )
    ...
    async def create_project(
        self, language: str, description: str, project_name: str = None
    ) -> Dict[str, Any]:
        ...
        log_coordinator(
            f"Iniciando criação de projeto {language.upper()}: {description[:100]}"
        )
        log_request(
            "coordinator",
            agent.name,
            f"Criar projeto: {description}",
            language=language,
        )
        ...
        if result_task.status == TaskStatus.COMPLETED:
            ...
            log_response(
                agent.name,
                "coordinator",
                f"Projeto criado com sucesso: {project_name or task.id}",
            )
        else:
    ...
    async def execute_code(
        self, language: str, code: str, run_tests: bool = True
    ) -> Dict[str, Any]:
        ...
        if not project_result.get("success"):
            log_error(
                "docker", f"Falha ao criar container: {project_result.get('error')}"
            )
            return project_result

        container_id = project_result["container_id"]
        log_docker_operation(
            "docker", "create_container", container_id, language=language
        )
    ...
    async def upload_and_process(
        ...
        result = {"upload": upload_result, "language": detected_lang}
    ...
    async def upload_zip_project(
        self, zip_content: bytes, project_name: str = None, auto_build: bool = False
    ) -> Dict[str, Any]:
        ...
        result = {"upload": upload_result, "language": language}
    ...
    async def push_to_github(
        ...
        log_github_operation(
            "coordinator",
            "push_to_github",
            f"{project_name} -> {repo_name or project_name}",
        )
        ...
        log_github_operation(
            "github", "collect_files", f"{len(files)} arquivos coletados"
        )
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "storage": storage,
            "llm_config": {"url": LLM_CONFIG["base_url"], "model": LLM_CONFIG["model"]},
        }
    ...
    async def analyze_project_requirements(self, description: str) -> Dict[str, Any]:
        ...
        return {"success": True, "requirement": requirement.to_dict()}

    async def generate_requirement_docs(
        self, req_id: str, doc_type: str = "full"
    ) -> Dict[str, Any]:
        """Gera documentacao para um requisito"""
        try:
            docs = await self.requirements_analyst.generate_documentation(
                req_id, doc_type
            )
    ...
    async def generate_requirement_tests(
        ...
        try:
            test_cases = await self.requirements_analyst.generate_test_cases(
                req_id, language
            )
            test_code = await self.requirements_analyst.generate_test_code(
                req_id, language
            )
            return {"success": True, "test_cases": test_cases, "test_code": test_code}
        except ValueError as e:
            return {"success": False, "error": str(e)}

    async def create_project_with_requirements(
        self, description: str, language: str, project_name: str = None
    ) -> Dict[str, Any]:
        ...
        await self.requirements_analyst.generate_documentation(
            requirement.id, "technical"
        )
        ...
        task = agent.create_task(
            ...
            {"requirement_id": requirement.id, "project_name": project_name},
        )
        result_task = await agent.execute_task(task.id)
        ...
        return {
            "success": result_task.status.value == "completed"
