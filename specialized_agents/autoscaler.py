...
class AgentAutoScaler:
        ...
        while self.running:
            try:
                ...
                self.metrics_history = [
                    m for m in self.metrics_history if m.timestamp.timestamp() > cutoff
                ]
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "last_scale_action": (
                self.last_scale_action.value if self.last_scale_action else None
            ),
            "last_scale_time": (
                datetime.fromtimestamp(self.last_scale_time).isoformat()
            ...
            "current_metrics": (
                {
                    ...
                    "memory_percent": (
                        recent_metrics.memory_percent if recent_metrics else 0
                    ),
                    "disk_percent": (
                        recent_metrics.disk_percent if recent_metrics else 0
                    ),
                }
                if recent_metrics
                else None
