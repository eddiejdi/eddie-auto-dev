...
class CleanupService:
    ...
    async def backup_item(
        ...
        try:
            ...
            meta_data = {
                ...
                "item_type": item_type,
                "retention_until": (datetime.now() + timedelta(days=self.backup_retention_days)).isoformat(),
                **(metadata or {})
            }
            ...
            for item in backup_dir.iterdir():
                ...
                if meta_path.exists():
                    try:
                        ...
                        retention_until = datetime.fromisoformat(meta.get("retention_until", ""))
    ...
    async def cleanup_unused_projects(
        self,
        days_inactive: int = 7
    ) -> CleanupReport:
            ...
            for line in result.stdout.strip().split('\n'):
                ...
                try:
                    ...
                    if logs.stdout or logs.stderr:
                        log_backup = self.backup_dirs["containers"] / f"{container_name}_logs.txt"
                    ...
                    report.errors.append(f"Erro ao remover container {container_name}: {e}")
    ...
    async def run_full_cleanup(self) -> CleanupReport:
        ...
        logger.info(f"Cleanup completo. Espaço liberado: {combined_report.space_freed_mb:.2f} MB")
    ...
    async def start_periodic_cleanup(self):
        ...
        logger.info(f"Cleanup periódico iniciado (intervalo: {self.cleanup_interval_hours}h)")
    ...
    def list_backups(self, item_type: str = None) -> List[Dict]:
        ...
        dirs_to_check = [self.backup_dirs[item_type]] if item_type else self.backup_dirs.values()
        ...
        return sorted(backups, key=lambda x: x.get("metadata", {}).get("backup_date", ""), reverse=True)
