...
class ConfluenceAgent:
    ...
    def create_from_template(
        self, template_name: str, title: str = None, **kwargs
    ) -> str:
        """Cria página a partir de um template"""
        if template_name not in self.templates:
            raise ValueError(
                f"Template não encontrado: {template_name}. Disponíveis: {list(self.templates.keys())}"
            )
...
{
            self.storage.paragraph(
                "Descreva o contexto e as forças em jogo que motivam esta decisão. Qual problema estamos tentando resolver?"
            )
        }
...
{
            self.storage.paragraph(
                "Uma breve descrição (2-3 sentenças) do que está sendo proposto."
            )
        }
...
{
            self.storage.paragraph(
                "Seção para registrar comentários e decisões durante a revisão."
            )
        }
...
Última atualização: {datetime.now().strftime("%Y-%m-%d %H:%M")} | Mantenedor: {
            kwargs.get("maintainer", "SRE Team")
        }
</div>
            ...
            self.storage.table(
                ...
                [
                    ...
                    [
                        "L3",
                        "Engineering Manager",
                        "Incidente P1 ou impacto crítico",
                        "@eng-manager",
                    ],
                ],
            )
...

{
            self.storage.table(
                ...
                [
                    ...
                    [
                        "👥 Participantes",
                        kwargs.get("participants", "@person1, @person2, @person3"),
                    ],
...
{self.storage.numbered_list(["Item 1", "Item 2", "Item 3"])}
...
{self.storage.bullet_list(["Próxima reunião: TBD", "Follow-up necessário: Item X"])}
"""
        self.set_content(page.id, content)
        return page
...
{
            self.storage.paragraph(
                kwargs.get("description", "Descrição breve do projeto e seu propósito.")
            )
        }
...
{
            self.storage.bullet_list(
                ["Fix: Descrição do bug corrigido", "Fix: Outro bug corrigido"]
            )
        }
...
{
            self.storage.paragraph(
                "Bem-vindo(a) ao time! 🎉 Este guia vai te ajudar nos primeiros dias."
            )
        }
...
<div class="success-box">
{self.storage.bullet_list(["Item positivo 1", "Item positivo 2", "Item positivo 3"])}
</div>
...
{
            self.storage.bullet_list(
                ["Ponto de melhoria 1", "Ponto de melhoria 2", "Ponto de melhoria 3"]
            )
        }
</div>
...
<div class="info-box">
{self.storage.bullet_list(["Ideia para experimentar 1", "Ideia para experimentar 2"])}
</div>
...
"""
    ...
    def validate_page(self, filepath: str) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_result["checks_passed"].append(
                f"Conteúdo: {len(content)} caracteres"
            )
            ...
            logger.info(
                f"Validação da página {filepath}: {'PASSOU' if validation_result['valid'] else 'FALHOU'}"
            )
    ...
    async def generate_from_description(
        self, description: str, doc_type: str = "auto"
    ) -> str:
        """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Confluence Agent - Gerador de Documentação"
    )
