...
with st.sidebar:
    ...
    if st.button('Send interceptor test message'):
        try:
            ...
            r = requests.post(f"{INTERCEPTOR_API}/communication/test", data={"message": msg}, timeout=5)
            if r.ok:
...
def format_conversations(convs):
    ...
    for c in convs:
        ...
        msg_count = c.get('message_count') or (len(c.get('messages', [])) if isinstance(c, dict) else '')
        preview = ''
...
st.markdown('Simple view: a read-only textbox with conversation summaries retrieved from the interceptor API.')
