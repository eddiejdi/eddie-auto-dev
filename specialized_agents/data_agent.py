...
class DataAgent:
    ...
    def validate_pipeline(self, pipeline: Pipeline) -> Dict[str, Any]:
        ...
        if not pipeline.sources:
            ...
            validation["checks"].append(
                f"✅ {len(pipeline.sources)} fontes configuradas"
            )
        ...
        for step in pipeline.steps:
            if step.input_source not in output_names and step.input_source not in [
                s.name for s in pipeline.sources
            ]:
                validation["valid"] = False
                validation["checks"].append(
                    f"❌ Step {step.id}: fonte '{step.input_source}' não encontrada"
                )
    ...
    def create_pipeline(self, name: str, description: str = "") -> Pipeline:
        ...
        pipeline = Pipeline(id=pipeline_id, name=name, description=description)
    ...
    def assess_data_quality(self, source_name: str) -> DataQualityReport:
        ...
        if isinstance(data[0], dict):
            ...
            for col in columns:
                null_count = sum(
                    1 for row in data if row.get(col) is None or row.get(col) == ""
                )
        ...
        null_penalty = (
            sum(null_counts.values()) / (total_rows * len(null_counts))
            if null_counts
            else 0
        )
    ...
    def convert_format(
        ...
        source = DataSource(name="temp_convert", format=input_format, path=input_path)
    ...
    def _execute_transform(self, step: PipelineStep, data: List[Dict]) -> List[Dict]:
        ...
        if step.type == TransformationType.FILTER:
            ...
            for row in data:
                ...
                try:
                    new_row[new_field] = eval(
                        expression, {"row": row, "__builtins__": {}}
                    )
                except:
            ...
            for row in data:
                key = (
                    tuple(row.get(f) for f in fields) if fields else tuple(row.values())
                )
                if key not in seen:
                    seen.add(key)
                    result.append(row)
...
if __name__ == "__main__":
    ...
    pipeline = agent.create_pipeline(
        name="test_pipeline", description="Pipeline de teste"
    )
