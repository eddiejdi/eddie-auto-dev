...
class DistributedCoordinator:
    ...
    def _load_precision_scores(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT language, total_tasks, successful_tasks, failed_tasks, avg_execution_time FROM agent_scores"
            )
            ...
            conn.close()
            logger.info(
                f"✓ Carregados scores de precisão para {len(self.precision_scores)} linguagens"
            )
    ...
    async def route_task(self, language: str, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"📊 {language}: Precisão={score.precision:.1f}% | Copilot={score.copilot_usage_ratio * 100:.0f}%"
        )
    ...
    async def _execute_on_agent(
        self, language: str, task: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Executa tarefa no agente especializador do homelab"""
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{self.homelab_url}/agents/{language}/execute", json=task
                )
            ...
            return {"success": False, "error": str(e), "executor": "agent"}

    async def _execute_on_copilot(
        self, language: str, task: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _record_success(self, language: str, result: Dict[str, Any]):
        ...
        logger.info(
            f"✅ Sucesso registrado para {language} (Precisão: {score.precision:.1f}%)"
        )
    ...
    def _record_failure(self, language: str):
        ...
        logger.warning(
            f"❌ Falha registrada para {language} (Precisão: {score.precision:.1f}%)"
        )
