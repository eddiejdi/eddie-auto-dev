...
def get_agent_stats(language: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "status": "🟢 Confiável" if score.precision >= 95 else "🟡 Bom" if score.precision >= 85 else "🟠 Aceitável" if score.precision >= 70 else "🔴 Baixo"
    }


@router.post("/record-result")
def record_result(language: str, success: bool, execution_time: float = 0.0) -> Dict[str, Any]:
