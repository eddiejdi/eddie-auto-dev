...
from .config import LANGUAGE_DOCKER_TEMPLATES, DATA_DIR, PROJECTS_DIR, BACKUP_DIR
...
"""
    ...
    async def create_project(
        ...
        project_name = (
            project_name or f"{language}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        run_args = [
            ...
            f"{project_path}:/app",
            "-p",
            f"{port}:{config.get('port_range', (8000,))[0]}",
            image_tag,
        ]
    ...
    async def run_code(
        ...
        if not container:
            return RunResult(
                success=False, error=f"Container {container_id} não encontrado"
            )
        ...
        result = self._run_docker_cmd(
            ["exec", container_id, *run_cmd.split(), filename], timeout=60
        )
    ...
    async def run_tests(
        ...
        if not container:
            return {
                "success": False,
                "error": f"Container {container_id} não encontrado",
            }
        ...
        result = self._run_docker_cmd(
            ["exec", container_id, *test_cmd.split()], timeout=120
        )
    ...
    async def update_code(
        self, container_id: str, code: str, filename: str = None
    ) -> bool:
    ...
    async def exec_command(
        ...
        result = self._run_docker_cmd(
            ["exec", container_id, "sh", "-c", command], timeout=timeout
        )
    ...
    async def install_packages(
        self, container_id: str, packages: List[str], language: str = None
    ) -> RunResult:
        ...
        result = self._run_docker_cmd(
            ["exec", container_id, "sh", "-c", f"{install_cmd} {' '.join(packages)}"],
            timeout=300,
        )
    ...
    async def get_logs(self, container_id: str, lines: int = 100) -> str:
        ...
        result = self._run_docker_cmd(["logs", "--tail", str(lines), container_id])
    ...
    async def remove_container(
        self, container_id: str, remove_project: bool = False, backup: bool = True
    ) -> bool:
        """Remove um container"""
        container = self.containers.get(container_id)
    ...
    async def export_project(
        self, container_id: str, output_path: Path = None
    ) -> Optional[str]:
        ...
        try:
            shutil.make_archive(
                str(output_path).replace(".zip", ""), "zip", project_path
            )
    ...
    async def import_project(
        ...
        project_name = (
            project_name or f"imported_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        project_path = PROJECTS_DIR / language / project_name
