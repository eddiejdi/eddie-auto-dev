...
class GitHubAgentClient:
    ...
    def __init__(self, api_url: str = None, token: str = None):
        self.api_url = api_url or GITHUB_AGENT_CONFIG.get(
            "api_url", "http://localhost:8080"
        )
        self.token = (
            token or GITHUB_AGENT_CONFIG.get("token") or os.getenv("GITHUB_TOKEN", "")
        )
    ...
    async def execute(self, action: str, params: Dict = None) -> GitHubResult:
        ...
        try:
            result = await self._request(
                "POST", "/api/execute", {"action": action, "params": params}
            )

            if "error" in result:
                return GitHubResult(success=False, action=action, error=result["error"])

            return GitHubResult(success=True, action=action, data=result)
        except Exception as e:
            return GitHubResult(success=False, action=action, error=str(e))
    ...
    async def clone_repo(
        self, owner: str, repo: str, destination: str = None
    ) -> GitHubResult:
        """Clona repositório"""
        return await self.execute(
            "clone_repo", {"owner": owner, "repo": repo, "destination": destination}
        )
    ...
    async def close_issue(
        ...
        """Fecha issue"""
        return await self.execute(
            "close_issue", {"owner": owner, "repo": repo, "issue_number": issue_number}
        )
    ...
    async def list_prs(
        ...
        """Lista pull requests"""
        return await self.execute(
            "list_prs", {"owner": owner, "repo": repo, "state": state}
        )
    ...
    async def get_file(
        ...
        return await self.execute(
            "get_file", {"owner": owner, "repo": repo, "path": path, "ref": ref}
        )
    ...
    async def search_code(
        self, query: str, owner: str = None, repo: str = None, language: str = None
    ) -> GitHubResult:
        """Busca código"""
        params = {"query": query}
        if owner:
    ...
    async def list_commits(
        self, owner: str, repo: str, branch: str = "main", limit: int = 10
    ) -> GitHubResult:
        """Lista commits"""
        return await self.execute(
            "list_commits",
    ...
    async def list_workflows(self, owner: str, repo: str) -> GitHubResult:
        ...
        return await self.execute("list_workflows", {"owner": owner, "repo": repo})
    ...
    async def trigger_workflow(
        ...
        return await self.execute(
            "trigger_workflow",
            {"owner": owner, "repo": repo, "workflow_id": workflow_id, "ref": ref},
        )
...
class GitHubWorkflow:
    ...
    async def create_project_repo(
        ...
        result = await self.client.create_repo(
            name=name, description=description, private=False, language=language
        )
    ...
    async def submit_code_for_review(
        ...
        if not branch_result.success:
            return {
                "success": False,
                "error": f"Erro ao criar branch: {branch_result.error}",
            }
        ...
        return {"success": True, "branch": branch_name, "pr": pr_result.data}
    ...
    async def fix_issue(
        ...
        # Criar branch
        await self.client.create_branch(owner=owner, repo=repo, branch_name=branch_name)
...
class DirectGitHubClient:
    ...
    async def check_connection(self) -> bool:
        ...
        try:
            response = await self.client.get(
                f"{self.api_url}/user", headers=self.headers
            )
    ...
    async def push_project(
        self, project_path: str, repo_name: str, description: str = ""
    ) -> Dict:
