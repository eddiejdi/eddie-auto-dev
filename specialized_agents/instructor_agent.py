...
class WebCrawler:
    ...
    async def __aenter__(self):
        timeout = aiohttp.ClientTimeout(
            total=INSTRUCTOR_CONFIG["request_timeout_seconds"]
        )
    ...
    def _parse_html(
        self, url: str, html: str, language: str
    ...
    def _detect_content_type(self, url: str, title: str, text: str) -> str:
        ...
        if any(
            x in url_lower
            for x in ["/docs/", "/documentation/", "/reference/", "/api/"]
        ):
            return "documentation"
        if any(
            x in url_lower
            for x in ["/tutorial/", "/guide/", "/learn/", "/getting-started"]
        ):
            return "tutorial"
...
class AgentInstructor:
    ...
    async def run_training_session(
        ...
        self.current_session = TrainingSession(
            id=session_id, started_at=datetime.now(), status=TrainingStatus.CRAWLING
        )
            ...
            async with WebCrawler() as crawler:
                for language in languages:
                    ...
                    if not await self._check_system_resources():
                        ...
                        logger.warning(
                            f"⏸️ Treinamento pausado (alta carga) em {language}"
                        )
    ...
    async def _train_language(self, crawler: WebCrawler, language: str):
        ...
        for url in sources.get("official_docs", []):
            if (
                self.current_session.pages_crawled
                >= self.config["max_pages_per_session"]
            ):
                break
    ...
    async def _index_contents(
        self, contents: List[CrawledContent], language: str
    ) -> int:
...
"""
    ...
    async def _save_contents_to_file(
        self, contents: List[CrawledContent], language: str
    ):
    ...
    async def _check_system_resources(self) -> bool:
        ...
        try:
            ...
            if cpu > self.config["cpu_threshold_percent"]:
                logger.debug(
                    f"CPU alta: {cpu}% > {self.config['cpu_threshold_percent']}%"
                )
                return False
    ...
    async def train_specific_language(
        self, language: str, query: Optional[str] = None
    ) -> Dict:
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "last_session": (
                self.session_history[-1].to_dict() if self.session_history else None
            ),
            "knowledge_sources": {
                lang: len(
                    sources.get("official_docs", []) + sources.get("frameworks", [])
                )
