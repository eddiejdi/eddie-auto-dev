...
def messages(conversation_id: str, limit: int, type: Optional[str]):
    ...
    try:
        ...
        response = requests.get(
            f"{API_BASE}/conversations/{conversation_id}/messages", params=params
        )
        response.raise_for_status()
        ...
        if msgs:
            ...
            for msg in msgs[-limit:]:
                print_colored(
                    f"[{msg['timestamp']}] {msg['message_type'].upper()}", "bold"
                )
...
def history(
    limit: int, agent: Optional[str], phase: Optional[str], hours: Optional[int]
):
...
@click.argument("conversation_id")
@click.option(
    "--format", type=click.Choice(["json", "markdown", "text"]), default="json"
)
...
def overview():
    ...
    try:
        ...
        print(
            f"{'Total de Mensagens:':<30} {interceptor['total_messages_intercepted']:,}"
        )
...
def content(query: str, limit: int):
    ...
    try:
        response = requests.get(
            f"{API_BASE}/search/by-content", params={"query": query, "limit": limit}
        )
        response.raise_for_status()
...
def monitor(interval: int):
    ...
    try:
        while True:
            ...
            print(
                f"📊 Mensagens: {interceptor['total_messages_intercepted']:,} | "
                + f"🔴 Ativas: {interceptor['active_conversations']} | "
                + f"✅ Completadas: {interceptor['total_conversations']}\n"
            )

            print(
                f"Buffer: {bus['buffer_size']}/{bus['buffer_max']} | "
            ...
            if active:
                ...
                for conv in active[:5]:
                    print(
                        f"  • {conv['participants']} | Fase: {conv['phase']} | "
