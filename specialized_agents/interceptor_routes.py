...
async def get_active_conversations(
    ...
    if agent:
        convs = [c for c in convs if any(agent.lower() in p.lower() for p in c["participants"])]
...
async def get_conversation_history(
    ...
    convs = interceptor.list_conversations(limit=limit, agent=agent, phase=phase, since=since)
...
async def finalize_conversation(conversation_id: str):
    ...
    return {
        "status": "success",
        "message": f"Conversa {conversation_id} finalizada"
    }
...
async def get_stats_by_phase():
    ...
    for phase in ConversationPhase:
        ...
        phases[phase.value] = {
            "count": len(convs),
            "avg_duration": sum(c["duration_seconds"] for c in convs) / len(convs) if convs else 0,
            "avg_messages": sum(c["message_count"] for c in convs) / len(convs) if convs else 0
        }
