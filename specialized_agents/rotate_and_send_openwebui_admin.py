...
def main():
    ...
    parser.add_argument(
        "--email", default="edenilson.adm@gmail.com", help="Admin email to update"
    )
    parser.add_argument(
        "--rounds", type=int, default=12, help="bcrypt rounds (work factor)"
    )
    parser.add_argument(
        "--chat-id", help="Telegram chat id (overrides /etc/eddie/telegram.env)"
    )
    parser.add_argument(
        "--token", help="Telegram bot token (overrides /etc/eddie/telegram.env)"
    )
    parser.add_argument(
        "--out",
        default="/tmp/openwebui_admin_password.txt",
        help="Path to write plaintext password",
    )
    ...
    try:
        ...
        print(
            "bcrypt not available. Install it and re-run the script.", file=sys.stderr
        )
        sys.exit(4)
    ...
    if not token or not chat:
        print(
            "Telegram token or chat_id not found; skipping send. Set /etc/eddie/telegram.env or pass --token/--chat-id"
        )
        sys.exit(0)
    ...
    try:
        ...
        print(
            "Could not parse Telegram response; check network and token. Password file left at",
            args.out,
        )
