...
def get_system_stats():
    ...
    try:
        ...
        return {
            ...
            "mem_percent": (
                round((total_mem - free_mem) / total_mem * 100, 1) if total_mem else 0
            ),
...
with st.sidebar:
    ...
    selected_lang = st.selectbox(
        "Linguagem", languages, format_func=lambda x: f"{x.upper()} Expert"
    )
    ...
    if st.button("🧹 Executar Limpeza", use_container_width=True):
        with st.spinner("Executando limpeza..."):
            ...
            st.success(
                f"Limpeza concluída! Espaço liberado: {report.get('space_freed_mb', 0):.2f} MB"
            )
    ...
    if st.session_state.current_agent:
        ...
        with col2:
            project_name = st.text_input(
                "Nome do Projeto (opcional)", key="create_project_name"
            )
        ...
        if st.button("🚀 Criar Projeto", type="primary"):
            if project_desc:
                with st.spinner("Criando projeto..."):
                    result = run_async(
                        st.session_state.manager.create_project(
                            agent.language, project_desc, project_name
                        )
                    )
...
with tab2:
        ...
        if upload_type == "Arquivo Único":
            ...
            if uploaded_file and st.button("📤 Fazer Upload"):
                with st.spinner("Processando..."):
                    ...
                    result = run_async(
                        st.session_state.manager.upload_and_process(
                            content, uploaded_file.name, lang, auto_run
                        )
                    )
            ...
            if uploaded_zip and st.button("📤 Upload ZIP"):
                with st.spinner("Extraindo e processando..."):
                    result = run_async(
                        st.session_state.manager.upload_zip_project(
                            uploaded_zip.read(), zip_project_name, auto_build
                        )
                    )
    ...
    with col2:
        ...
        # Listar projetos
        download_lang = st.selectbox(
            "Filtrar por linguagem", ["Todas"] + languages, key="dl_lang"
        )
        ...
        if "available_projects" in st.session_state:
            selected_project = st.selectbox(
                "Selecione projeto", st.session_state.available_projects
            )
            ...
            if selected_project and st.button("📥 Download como ZIP"):
                ...
                zip_data = run_async(
                    st.session_state.manager.download_project(lang, name)
                )

                if zip_data:
                    st.download_button(
                        "💾 Salvar ZIP", zip_data, f"{name}.zip", "application/zip"
                    )
                else:
...
with tab3:
                ...
                with st.expander(f"🐳 {container['name']} ({container['status']})"):
                    ...
                    with col_a:
                        if container["status"] == "running":
                            if st.button(
                                "⏹️ Parar", key=f"stop_{container['container_id']}"
                            ):
                                run_async(
                                    st.session_state.manager.docker.stop_container(
                            ...
                            if st.button(
                                "▶️ Iniciar", key=f"start_{container['container_id']}"
                            ):
                                run_async(
                                    st.session_state.manager.docker.start_container(
                                        container["container_id"]
                                    )
                                )
                                st.rerun()
                    ...
                    with col_b:
                        if st.button(
                            "📋 Logs", key=f"logs_{container['container_id']}"
                        ):
                            logs = run_async(
                                st.session_state.manager.docker.get_logs(
                                    container["container_id"]
                                )
                            )
                            st.code(logs)
                    ...
                    with col_c:
                        if st.button(
                            "🗑️ Remover", key=f"rm_{container['container_id']}"
                        ):
                            run_async(
                                st.session_state.manager.docker.remove_container(
                                    container["container_id"], backup=True
                                )
                            )
                            st.rerun()
        else:
    ...
    with col2:
        ...
        new_deps = st.text_input(
            "Dependências (separadas por vírgula)", key="docker_deps"
        )
        ...
        if st.button("🐳 Criar Container"):
            if new_code:
                ...
                with st.spinner("Criando container..."):
                    result = run_async(
                        st.session_state.manager.docker.create_project(
                            new_lang, new_code, deps
                        )
                    )
        ...
        exec_container = st.selectbox(
            "Container", [c["container_id"] for c in containers] if containers else []
        )
        ...
        if st.button("▶️ Executar") and exec_container:
            with st.spinner("Executando..."):
                result = run_async(
                    st.session_state.manager.docker.exec_command(
                        exec_container,
                        (
                            f"python -c '{exec_code}'"
                            if "python" in exec_container
                            else exec_code
                        ),
                    )
                )
...
with tab4:
    ...
    with col1:
        ...
        search_lang = st.selectbox(
            "Filtrar linguagem", ["Todas"] + languages, key="rag_lang"
        )
            ...
            with st.spinner("Buscando..."):
                ...
                if results:
                    for i, result in enumerate(results, 1):
                        with st.expander(
                            f"Resultado {i} - Score: {result.get('score', 0):.3f}"
                        ):
    ...
    with col2:
        ...
        index_type = st.radio(
            "Tipo de Conteúdo", ["Código", "Documentação", "Conversa"]
        )
        ...
        if index_type == "Código":
            ...
            if st.button("📥 Indexar Código"):
                ...
                success = run_async(
                    manager.index_code(index_code, index_lang, index_desc)
                )
            ...
            if st.button("📥 Indexar Documentação"):
                ...
                success = run_async(
                    manager.index_documentation(index_content, index_title)
                )
            ...
            if st.button("📥 Indexar Conversa"):
                ...
                success = run_async(
                    manager.index_conversation(index_question, index_answer)
                )
...
with tab5:
    ...
    with col1:
        ...
        if st.button("📋 Listar Repos") and github_owner:
            with st.spinner("Buscando..."):
                result = run_async(
                    st.session_state.manager.github_client.list_repos(github_owner)
                )
                ...
                if result.success:
                    ...
                    for repo in repos[:10]:
                        st.write(
                            f"• **{repo.get('name')}** - {repo.get('description', 'Sem descrição')[:50]}"
                        )
                else:
        ...
        new_repo_lang = st.selectbox(
            "Linguagem Principal", languages, key="new_repo_lang"
        )
        ...
        if st.button("🚀 Criar Repositório"):
            if new_repo_name:
                with st.spinner("Criando..."):
                    result = run_async(
                        st.session_state.manager.github_client.create_repo(
                            new_repo_name,
                            new_repo_desc,
                            new_repo_private,
                            new_repo_lang,
    ...
    with col2:
        ...
        push_repo_name = st.text_input(
            "Nome no GitHub (deixe vazio para usar nome do projeto)",
            key="push_repo_name",
        )
        ...
        if st.button("📤 Push para GitHub") and push_project:
            ...
            with st.spinner("Fazendo push..."):
                result = run_async(
                    st.session_state.manager.push_to_github(
        ...
        if st.button("📋 Listar Issues"):
            if issue_owner and issue_repo:
                result = run_async(
                    st.session_state.manager.github_client.list_issues(
...
with tab6:
    ...
    with col1:
        ...
        if st.button("💾 Salvar Configurações"):
            st.session_state.manager.cleanup_service.backup_retention_days = (
                retention_days
            )
            st.session_state.manager.cleanup_service.cleanup_interval_hours = (
                cleanup_interval
            )
        ...
        if st.button("📊 Verificar Armazenamento"):
            storage = run_async(
                st.session_state.manager.cleanup_service.get_storage_status()
            )
...
with tab7:
    ...
    with col2:
        st.metric(
            "💾 RAM", f"{stats.get('mem_used_gb', 0)}/{stats.get('mem_total_gb', 0)} GB"
        )
    with col3:
...
with tab8:
    ...
    def fetch_comm_stats():
        try:
            r = requests.get("http://localhost:8503/communication/stats", timeout=5)
    ...
    with control_col1:
        ...
        try:
            stats = requests.get(
                "http://localhost:8503/communication/stats", timeout=3
            ).json()
        ...
        if is_recording:
            if st.button("⏸️ Pausar Gravação", use_container_width=True):
                requests.post("http://localhost:8503/communication/pause", timeout=3)
                st.cache_data.clear()
                st.rerun()
        else:
            if st.button("▶️ Retomar Gravação", use_container_width=True):
                requests.post("http://localhost:8503/communication/resume", timeout=3)
                st.cache_data.clear()
                st.rerun()

    with control_col2:
        if st.button("🗑️ Limpar Log", use_container_width=True):
            requests.post("http://localhost:8503/communication/clear", timeout=3)
            st.cache_data.clear()
    ...
    with control_col4:
        auto_refresh = st.checkbox(
            "Auto-refresh (5s)", value=False, key="comm_auto_refresh"
        )
    ...
    with stats_container:
        ...
        if not comm_stats:
            st.error(
                "❌ Não foi possível conectar à API. Verifique se está rodando na porta 8503."
            )
        else:
            ...
            with stats_col1:
                ...
                st.metric(
                    "📊 Buffer",
                    f"{comm_stats.get('buffer_size', 0)}/{comm_stats.get('buffer_max', 1000)}",
                )
                uptime = comm_stats.get("uptime_seconds", 0)
                st.metric(
                    "⏱️ Uptime", f"{int(uptime // 3600)}h {int((uptime % 3600) // 60)}m"
                )
                st.metric(
                    "📈 Msgs/min", f"{comm_stats.get('messages_per_minute', 0):.2f}"
                )
        ...
        for col_idx, (filter_key, filter_label) in enumerate(row_filters):
            with cols[col_idx]:
                ...
                new_value = st.checkbox(
                    filter_label, value=is_active, key=f"cb_{filter_key}"
                )
    ...
    with messages_container:
        ...
        messages_data = fetch_comm_messages(
            limit=limit_messages, source=source_filter if source_filter else None
        )
        ...
        if search_term:
            messages = [
                ...
                or search_term.lower() in m.get("source", "").lower()
                or search_term.lower() in m.get("target", "").lower()
            ]
        ...
        if messages:
            ...
            st.info(
                f"📊 Mostrando {len(messages)} de {comm_stats.get('total_messages', len(messages))} mensagens"
            )
        else:
            st.info(
                "📭 Nenhuma mensagem capturada ainda. As mensagens aparecerão aqui quando os agentes começarem a se comunicar."
            )
    ...
    with export_col1:
        ...
        export_format = st.selectbox(
            "Formato", ["JSON", "Markdown"], key="export_format_select"
        )

        if st.button("📥 Exportar", key="btn_export"):
            try:
                resp = requests.get(
                    f"{API_BASE_URL}/communication/export?format={export_format.lower()}",
                    timeout=10,
                )
                if resp.status_code == 200:
                    ...
                    st.download_button(
                        ...
                        (
                            f"application/{export_format.lower()}"
