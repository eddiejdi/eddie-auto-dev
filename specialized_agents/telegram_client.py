...
class TelegramConfig:
    ...
    def from_env(cls) -> "TelegramConfig":
        ...
        try:
            ...
            raise RuntimeError(
                "Telegram bot token must be stored in the repo cofre (eddie/telegram_bot_token)"
            )
...
class TelegramClient:
    ...
    async def _request(self, method: str, data: Dict) -> Dict[str, Any]:
        ...
        try:
            response = await self.client.post(f"{self.api_url}/{method}", json=data)
    ...
    async def send_message(
        ...
        try:
            ...
            if (
                ...
                and "message thread not found" in err.lower()
            ):
    ...
    async def send_document(
        self, document_path: str, caption: str = None, chat_id: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            with open(document_path, "rb") as f:
                ...
                response = await self.client.post(
                    f"{self.api_url}/sendDocument", data=data, files=files
                )
                result = response.json()
    ...
    async def send_photo(
        self, photo_path: str, caption: str = None, chat_id: str = None
    ) -> Dict[str, Any]:
        """Envia foto"""
        use_bus = os.getenv("TELEGRAM_USE_BUS", "0").lower() in ("1", "true", "yes")
        if not getattr(self, "force_direct", False) and use_bus:
        ...
        try:
            with open(photo_path, "rb") as f:
                ...
                response = await self.client.post(
                    f"{self.api_url}/sendPhoto", data=data, files=files
                )
                result = response.json()
...
class TelegramNotifier:
    ...
    async def notify_deploy(
        self, project: str, status: str, url: str = None, details: str = None
    ) -> Dict:
    ...
    async def notify_github(
        self, action: str, repo: str, url: str, details: str = None
    ) -> Dict:
    ...
    async def notify_training(
        self, model: str, status: str, conversations: int = 0, time_elapsed: float = 0
    ) -> Dict:
    ...
    async def notify_agent_task(
        self, agent: str, task: str, status: str, result_url: str = None
    ) -> Dict:
