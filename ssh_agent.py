...
class SSHAgentManager:
    ...
    def add_host(
        ...
        return {
            "status": "success",
            "message": f"Host '{name}' adicionado",
            "host": host.to_dict(),
        }
            ...
            if action == "exit" or action == "quit":
                ...
                if result["hosts"]:
                    for h in result["hosts"]:
                        ...
                        print(
                            f"  {status} {h['name']}: {h['username']}@{h['hostname']}:{h['port']}"
                        )
                        if h["description"]:
                ...
                if len(args) >= 3:
                    ...
                    result = ssh_manager.add_host(
                        name, hostname, username, port, key_file
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exec", nargs=2, metavar=("HOST", "CMD"), help="Executa comando"
    )
