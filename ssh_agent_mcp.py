...
class MCPServer:
    ...
    def tool_connect_new(self, arguments: dict) -> dict:
        ...
        try:
            ...
            # Testa a conexão
            stdin, stdout, stderr = client.exec_command("hostname && echo 'Conexão estabelecida!'")
    ...
    def tool_interactive_connect(self, arguments: dict) -> dict:
        ...
        if missing:
            ...
            if not hostname:
                response["ask_hostname"] = "Qual o IP ou hostname do servidor? (ex: 192.168.1.100, servidor.exemplo.com)"
            if not username:
                response["ask_username"] = "Qual o nome de usuário para login SSH? (ex: root, admin, ubuntu)"
            
            if saved_hosts:
                response["tip"] = f"Dica: Existem {len(saved_hosts)} hosts salvos. Use ssh_list_hosts para ver."
        ...
        if not password:
            ...
            try:
                client = paramiko.SSHClient()
                client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                client.connect(hostname=hostname, port=port, username=username, timeout=10)
        ...
        try:
            if method == 'initialize':
                ...
                if tool_name in self.tools:
                    ...
                    return self.error(req_id, -32601, f"Tool não encontrada: {tool_name}")
