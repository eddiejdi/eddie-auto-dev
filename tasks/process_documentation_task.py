...
Seja conciso e técnico."""
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            ...
            return {"agent": agent_name, "interview": f"Erro: {e}", "status": "error"}
...
async def main():
    ...
    # Fase 1: Entrevistas
    await send_telegram(
        "🎬 <b>Fase 1: Iniciando Entrevistas</b>\n\nTotal de agents: "
        + str(len(agents_to_interview))
    )
    ...
    for i, agent in enumerate(agents_to_interview, 1):
        ...
        if i % 5 == 0:
            await send_telegram(
                f"📊 Progresso: {i}/{len(agents_to_interview)} entrevistas concluídas"
            )
    ...
    try:
        await git_commit_and_push(
            "docs: atualizar documentação com entrevistas dos agents"
        )
        await send_telegram(
