    ...
    for conv in data.get("conversations", []):
        ...
        if msg_response.status_code == 200:
            ...
            for msg in messages_data.get('messages', [])[:5]:  # Mostrar primeiras 5
                print(f"    - [{msg.get('type')}] {msg.get('source')} → {msg.get('target')}: {msg.get('content', '')[:50]}")
        print()
else:
...
print(f"Total de mensagens interceptadas: {stats['interceptor']['total_messages_intercepted']}")
