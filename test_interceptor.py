...
def print_test(name: str, status: bool, message: str = ""):
    ...
    icon = (
        f"{Colors.GREEN}✅{Colors.RESET}" if status else f"{Colors.RED}❌{Colors.RESET}"
    )
    print(f"{icon} {name}")
    if message:
...
def test_communication_bus():
    ...
    print_test(
        "Publicar mensagem", msg is not None, f"Message ID: {msg.id if msg else 'None'}"
    )
    ...
    print_test(
        ...
        len(messages) > 0,
        f"Buffer size: {len(bus.message_buffer)}",
    )
...
def test_interceptor():
    ...
    print_test(
        "Conversa capturada",
        conv_id in interceptor.active_conversations,
    ...
    print_test(
        "Mensagens coletadas",
        conv is not None and conv["message_count"] >= 5,
...
def test_api_endpoints():
    ...
    try:
        ...
        try:
            ...
            print_test("GET /conversations/active", False, f"Erro: {str(e)[:50]}")
        ...
        try:
            ...
            print_test("GET /stats", False, f"Erro: {str(e)[:50]}")
        ...
        try:
            ...
            print_test("GET /stats/by-phase", False, f"Erro: {str(e)[:50]}")
        ...
        try:
            ...
            print_test("GET /stats/by-agent", False, f"Erro: {str(e)[:50]}")
        ...
        try:
            ...
            print_test("GET /conversations/history", False, f"Erro: {str(e)[:50]}")

    except ImportError:
        print(
            f"{Colors.YELLOW}⚠️  Requests não instalado - pulando testes de API{Colors.RESET}"
        )
...
def test_performance():
    ...
    print_test("Throughput", throughput > 100, f"{throughput:.0f} mensagens/segundo")

    # Teste 2: Tamanho do buffer
    stats = bus.get_stats()
    print_test(
        "Buffer circular",
        stats["buffer_size"] <= stats["buffer_max"],
...
def test_database():
    ...
    print_test("Banco de dados criado", db_path.exists(), f"Caminho: {db_path}")
    ...
    try:
        ...
        print_test("Tabelas necessárias", all_exist, f"Tabelas: {', '.join(tables)}")

        # Teste 3: Índices criados
        cursor.execute("SELECT name FROM sqlite_master WHERE type='index'")
        indices = [row[0] for row in cursor.fetchall()]

        print_test("Índices criados", len(indices) > 0, f"Total: {len(indices)}")
...
def test_cli():
    ...
    try:
        result = subprocess.run(
            [sys.executable, cli_path, "--help"], capture_output=True, timeout=5
        )
        print_test(
            "CLI --help", result.returncode == 0, f"Return code: {result.returncode}"
        )
    except Exception as e:
...
def test_streamlit_dashboard():
    ...
    print_test("Arquivo do dashboard existe", exists, f"Caminho: {dashboard_path}")
    ...
    try:
        ...
        print_test(
            "Dependências do Streamlit", True, "Todas as bibliotecas disponíveis"
        )
    except ImportError as e:
        print_test("Dependências do Streamlit", False, f"Faltando: {str(e)}")
...
def main():
    ...
    if passed == total:
        ...
        print(
            f"{Colors.YELLOW}⚠️  Alguns testes falharam - verificar detalhes acima{Colors.RESET}\n"
        )
        return 1
