...
def test_cumulative_pnl_panel_uses_prometheus_cumulative_metrics() -> None:
    """Painel 11 deve voltar a usar as séries acumuladas do exporter."""
    panel = get_panel(11)
    targets = panel["targets"]
    assert panel["datasource"]["type"] == "prometheus"
    assert panel["datasource"]["uid"] == "dfc0w4yioe4u8e"
    assert len(targets) == 2
    assert targets[0]["expr"] == 'max without(job, instance, coin, exported_coin, exported_profile) ((btc_trading_cumulative_pnl{coin="$coin",profile=~"$profile"}) or (btc_trading_cumulative_pnl{exported_coin="$coin",profile=~"$profile"}))'
    assert targets[0]["legendFormat"] == "PnL Total"
    assert targets[1]["expr"] == 'max without(job, instance, coin, exported_coin, exported_profile) ((btc_trading_cumulative_pnl_24h{coin="$coin",profile=~"$profile"}) or (btc_trading_cumulative_pnl_24h{exported_coin="$coin",profile=~"$profile"}))'
    assert targets[1]["legendFormat"] == "PnL 24h"
