...
def main(poll_seconds: int = 3):
    ...
    def on_message(msg):
        ...
        logging.info(line)
        print(
            f"[bus] {data.get('type')} {data.get('source')}→{data.get('target')} id={data.get('id')}"
        )
    ...
    while True:
        try:
            ...
            for m in msgs:
                ...
                line = json.dumps(
                    {"bus_snapshot": data}, default=str, ensure_ascii=False
                )
                logging.info(line)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Agent bus listener (polls every N seconds)"
    )
