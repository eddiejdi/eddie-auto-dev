...
class CatalogUpdater:
    ...
    def detect_changes(self) -> dict:
        """Detect changes across taxonomy domains (variables + tables + apis)."""
        old_catalog = self.load_catalog()

        # Full taxonomy orchestrator (variables, tables, apis + reports)
        logger.info("Running taxonomy scanners (variables, tables, apis)...")
        try:
            result = run(
        ...
        # Compare variables catalog (primary) + report domain totals
        new_catalog = self.load_catalog()
        domain_totals = {}
        for domain, path, key in (
            ("variables", self.catalog_file, "totalVariables"),
            ("tables", self.project_root / ".tables-catalog" / "catalog.json", "totalTables"),
            ("apis", self.project_root / ".apis-catalog" / "catalog.json", "totalEndpoints"),
        ):
            try:
                if path.exists():
                    domain_totals[domain] = json.loads(path.read_text()).get("metadata", {}).get(key, 0)
            except Exception:
        ...
        if not old_catalog:
            ...
            return {
                "type": "initial",
                "new_variables": domain_totals.get("variables", 0),
    ...
    def generate_reports(self):
        """Generate all reports (handled by catalog_taxonomy orchestrator)."""
        logger.info("Reports are generated by catalog_taxonomy.py")
        return True

    def commit_changes(self, message: str) -> bool:
        """Commit catalog changes to git."""
        try:
            # Stage catalog files (all taxonomy domains)
            run(
                [
                    "git",
                    "add",
