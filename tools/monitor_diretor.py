...
def run_check_scripts():
    ...
    for c in CHECK_SCRIPTS:
        try:
            p = subprocess.run(
                ["python3", c], capture_output=True, text=True, timeout=120
            )
            outputs.append(f"\n== {c} ==\n{p.stdout}\n{p.stderr}")
...
def main(timeout_seconds: int = 600):
    ...
    seen = set()
    print(
        "Monitor started, listening for DIRETOR responses until", deadline.isoformat()
    )
        ...
        if new:
            for m in new:
                ...
                if any(
                    ...
                    log_response(
                        "assistant", "DIRETOR", summary, original_message_id=m.id
                    )
                    # If tests failed, send an error log
                    if (
                        "FAILED" in diag_text
                        or "Traceback" in diag_text
                        or "ERROR" in diag_text
                    ):
                        log_error(
                            "assistant",
                            "Diagnostics found failing tests or errors; see /tmp/diretor_diagnostics.txt",
                        ...
                        log_response(
                            "assistant",
                            "DIRETOR",
                            "Diagnostics completed with no test failures.",
                            original_message_id=m.id,
                        )
                else:
                    log_response(
                        "assistant",
                        "DIRETOR",
                        "Received your message, thank you. No error keywords detected.",
                        original_message_id=m.id,
                    )
        time.sleep(5)
