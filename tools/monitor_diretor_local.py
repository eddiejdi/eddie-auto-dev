...
def load_bus():
    ...
    return agent_bus.get_communication_bus(), agent_bus.MessageType, agent_bus.log_response, agent_bus.log_error
...
def main(timeout: int = 60):
    ...
    seen = set()
    print(f"Monitor started, listening for DIRETOR responses until {deadline.isoformat()}")
    while datetime.now() < deadline:
        msgs = bus.get_messages(limit=200, message_types=[MessageType.RESPONSE, MessageType.ERROR], source='DIRETOR')
