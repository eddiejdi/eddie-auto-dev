...
logging.basicConfig(filename="operation_agent_audit.log", level=logging.INFO,
...
async def evoke(payload: EvokePayload, authorization: str | None = Header(None)):
    ...
    if not dev_endpoint or not test_endpoint:
        raise HTTPException(status_code=500, detail="DEV_AGENT_ENDPOINT or TEST_AGENT_ENDPOINT not configured")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            r = await client.post(f"{dev_endpoint.rstrip('/')}/evoke", json={
                "chat_session": payload.chat_session,
                "context": {"source": "operation_agent", "workflow": payload.workflow, "run_id": payload.run_id}
            })
            results["invocations"]["dev_agent"] = {"status_code": r.status_code, "body": r.text}
        except Exception as e:
            results["invocations"]["dev_agent"] = {"error": str(e)}

        # Invoke tester agent
        try:
            r = await client.post(f"{test_endpoint.rstrip('/')}/evoke", json={
                "chat_session": payload.chat_session,
                "context": {"source": "operation_agent", "workflow": payload.workflow, "run_id": payload.run_id}
            })
            results["invocations"]["test_agent"] = {"status_code": r.status_code, "body": r.text}
        except Exception as e:
        ...
        if training_endpoint and payload.responsible_agents:
            try:
                r = await client.post(f"{training_endpoint.rstrip('/')}/penalize_and_retrain", json={
                    "agents": payload.responsible_agents,
                    "reason": f"Workflow failure {payload.workflow} run {payload.run_id}",
                    "origin": "operation_agent"
                })
                results["invocations"]["training_agent"] = {"status_code": r.status_code, "body": r.text}
            except Exception as e:
                results["invocations"]["training_agent"] = {"error": str(e)}

    logging.info("Evoke called: repo=%s workflow=%s run_id=%s branch=%s sha=%s chat=%s agents=%s",
