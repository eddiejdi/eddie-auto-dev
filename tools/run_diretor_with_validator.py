...
# load diretor module
spec = importlib.util.spec_from_file_location('run_diretor', str(repo / 'dev_agent' / 'run_diretor_service.py'))
diretor_mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(diretor_mod)

# load validator module
spec2 = importlib.util.spec_from_file_location('auto_validator', str(repo / 'tools' / 'auto_validate_redirect.py'))
val_mod = importlib.util.module_from_spec(spec2)
spec2.loader.exec_module(val_mod)

# load bus for attach
spec_bus = importlib.util.spec_from_file_location('agent_bus_local', str(repo / 'specialized_agents' / 'agent_communication_bus.py'))
