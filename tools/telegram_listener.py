...
async def main():
    ...
    while True:
        ...
        updates = await client.get_updates(
            offset=(last_update_id + 1) if last_update_id else None, limit=10
        )
            ...
            for upd in updates["data"]:
                ...
                if text.startswith("/") or text.endswith("?"):
                    ...
                    bus.publish(
                        ...
                        "DIRETOR",
                        text,
                        {"from_user": user, "chat_id": chat_id},
                    )
                    await notifier.notify_info(
                        "Pergunta recebida", f"Encaminhada ao DIRETOR: {text}"
                    )
                else:
        ...
        for resp in responses:
            ...
            await client.send_message(
                f"Resposta do DIRETOR:\n{content}", chat_id=chat_id
            )
            print(
                f"[telegram_listener] Resposta enviada ao Telegram para chat_id={chat_id}"
            )
