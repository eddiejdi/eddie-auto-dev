...
async function fetchTestMasses(request) {
  ...
  async function fetchRecordsFrom(endpointPath) {
    ...
    if (!response.ok) {
      ...
      const bodyPreview = body.trimStart().startsWith('<') ? '' : body.slice(0, 120);
      const detail = bodyPreview ? `: ${bodyPreview}` : '';
      const error = new Error(`API retornou ${response.status}${detail}`);
...
async function getMasses(request) {
  ...
  try {
    ...
    if (cache.records.length) {
      return { records: cache.records, fetchedAt: cache.fetchedAt, source: 'cache', apiError: error.message };
    }
    throw error;
  }
}
