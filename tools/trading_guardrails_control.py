...
def _extract_database_url_from_systemctl(output: str) -> str | None:
...
def _ensure_database_url() -> None:
    ...
    try:
        result = subprocess.run(
            ["systemctl", "show", "crypto-agent@BTC_USDT_aggressive.service", "-p", "Environment", "--value"],
            check=True,
            capture_output=True,
            text=True,
        )
        runtime_url = _extract_database_url_from_systemctl(result.stdout.strip())
        if runtime_url:
    ...
    if env_path.exists():
        match = re.search(r"^DATABASE_URL=(.+)$", env_path.read_text(encoding="utf-8"), re.MULTILINE)
        if match:
            os.environ["DATABASE_URL"] = match.group(1).strip()
