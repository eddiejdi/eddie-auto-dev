...
DATABASE_URL = os.environ.get('DATABASE_URL', 'postgresql://postgress:eddie_memory_2026@localhost:55432/autocoinbot')
...
def main(timeout=None):
    ...
    print(f"Watching Postgres for coordinator messages since {last_seen.isoformat()} (poll {POLL_INTERVAL}s)")
    conn = connect()
