...
def extract_from_claude_dev():
    ...
    if not os.path.exists(CLAUDE_DEV_DIR):
        print("⚠️ Diretório Claude Dev não encontrado")
        return conversations
...
def extract_from_chats_raw():
    ...
    if not CHATS_DIR.exists():
        print("⚠️ Diretório chats_raw não encontrado")
        return conversations
...
def save_conversations(conversations, filename=None):
    ...
    if not filename:
        filename = f"/home/homelab/myClaude/training_data/training_{TODAY}.jsonl"
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if r2.status_code == 200:
                    success += len(batch)
                    print(
                        f"  ✅ Batch {i // batch_size + 1}: {len(batch)} docs (default)"
                    )
                else:
...
def learn_via_ollama(conversations):
    ...
    context_file = f"/home/homelab/myClaude/training_data/knowledge_{TODAY}.txt"
