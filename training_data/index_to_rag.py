                    ...
                    if isinstance(resp, list):
                        ...
                        if "value" in resp:
                            ...
                            response_text = resp.get(
                                "content", resp.get("text", str(resp))
                            )
                    else:
    ...
    for i in range(0, len(documents), batch_size):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"  ✅ Batch {i // batch_size + 1}: {len(batch)} documentos indexados"
                )
            else:
                print(
...
def learn_via_agent(conversations):
    ...
    try:
        response = requests.post(
            ...
            json={"knowledge": knowledge, "source": "vscode_chats", "date": TODAY},
            timeout=120,
        )
...
def main():
    ...
    print("\n💡 Para consultar, use:")
    print(
        f'   curl -X POST "{RAG_API}/rag/search" -H "Content-Type: application/json" -d \'{{"query": "sua pergunta"}}\''
    )
