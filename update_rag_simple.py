...
def index_to_chromadb():
    ...
    try:
        ...
        collection = client.create_collection(
            name="eddie_knowledge_v2", metadata={"hnsw:space": "cosine"}
        )
    ...
    for item in KNOWLEDGE:
        ...
        if embedding:
            collection.upsert(
                ...
                metadatas=[
                    {"topic": item["topic"], "date": datetime.now().isoformat()}
                ],
            )
            added += 1
...
def save_training_jsonl():
    ...
    pairs = [
        (
            ...
            "Use openwebui_integration.py que conecta ao Ollama (192.168.15.2:11434) com seleção automática de modelo.",
        ),
        (
            "Qual modelo usar para assistente pessoal?",
            "Use eddie-assistant baseado em dolphin-llama3:8b, sem censura para uso pessoal.",
        ),
        (
            "Como configurar WAHA para WhatsApp?",
            "docker run -d --name waha -p 3001:3000 -e WAHA_NO_API_KEY=True devlikeapro/waha:latest. Dashboard em :3001/dashboard",
        ),
        (
            "Como criar modelo restrito para código?",
            "Use Modelfile com SYSTEM prompt restritivo. Exemplo: eddie-coder-strict.Modelfile",
        ),
        (
            "Quais comandos do Telegram bot?",
            "/models (lista), /profiles (perfis), /profile <nome>, /use <modelo>, /auto_profile",
        ),
        (
            "O QR Code do WhatsApp expira rápido?",
            "Use engine WEBJS: -e WHATSAPP_DEFAULT_ENGINE=WEBJS. Mais estável que NOWEB.",
        ),
        (
            "Diferença eddie-assistant e eddie-coder?",
            "eddie-assistant (dolphin) sem censura para uso pessoal. eddie-coder (qwen) restrito a código.",
        ),
        (
            "Onde ficam os serviços?",
            "Ollama: 192.168.15.2:11434, Open WebUI: :3000, WAHA: :3001, Streamlit: localhost:8502",
        ),
    ]

    filename = (
        TRAINING_DIR / f"training_{datetime.now().strftime('%Y-%m-%d')}_knowledge.jsonl"
    )
    with open(filename, "w") as f:
        for q, a in pairs:
            f.write(
                json.dumps({"prompt": q, "completion": a}, ensure_ascii=False) + "\n"
            )
