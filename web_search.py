...
class WebSearchEngine:
    ...
    def search_and_extract(
        self, query: str, num_results: int = 3, extract_content: bool = True
    ) -> List[SearchResult]:
        """
    ...
    def save_to_rag(self, results: List[SearchResult], query: str) -> Dict:
        ...
        for result in results:
            try:
                ...
                doc_id = hashlib.md5(
                    f"{result.url}{result.title}".encode()
                ).hexdigest()[:12]
...
if __name__ == "__main__":
    ...
    results = engine.search_and_extract(
        "Python machine learning tutorial", num_results=3
    )
    ...
    # Formatar para LLM
    formatted = engine.format_results_for_llm(
        results, "Python machine learning tutorial"
    )
    print(formatted)
