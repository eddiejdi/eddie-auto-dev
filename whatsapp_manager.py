...
# Configurar página
st.set_page_config(page_title="WhatsApp Bot Manager", page_icon="📱", layout="wide")
...
def main():
    ...
    # Header
    st.markdown(
        '<div class="main-header">📱 WhatsApp Bot Manager</div>', unsafe_allow_html=True
    )
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📊 Status", "📷 QR Code", "💬 Enviar Mensagem", "📋 Chats"]
    )
    ...
    with tab3:
        ...
        if session_status not in ["CONNECTED", "WORKING", "READY"]:
            st.warning(
                "⚠️ WhatsApp não está conectado. Conecte primeiro na aba 'QR Code'"
            )
        else:
            with st.form("send_message"):
                ...
                submitted = st.form_submit_button(
                    "📤 Enviar Mensagem", use_container_width=True
                )
